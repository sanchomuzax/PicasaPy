#!/bin/bash
# PicasaPy SessionStart hook — Claude Code.
#
# Vékony belépő: a tényleges munkát a `scripts/bootstrap_env.sh` végzi, közösen
# a Codex-hookkal (`.codex/hooks/session-start.sh`). Korábban a két hook a
...
exec "$(dirname "${BASH_SOURCE[0]}")/../../scripts/bootstrap_env.sh" .claude/skills
