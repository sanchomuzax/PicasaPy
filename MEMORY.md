...
- **2026-07-20: Windows CI-láb — `pytest-timeout` + ritkítás** (ADR-002,
  `docs/decisions/ci-windows-strategy.md`) — a 138 eddigi
  `windows-latest` futásból 38 (28%) megszakadt/beragadt (#53 GIL-deadlock
  osztály, ismert Qt/pytest-qt community-jelenség, nem PicasaPy-specifikus).
  Mostantól: (1) `pytest-timeout` mindkét lábon (`--timeout=60
  --timeout-method=thread`) — egyedi teszt akad be, ne az egész job; (2) a
  `windows-latest` láb külön jobban, csak `push` (=main-re kerülés) eseményen
  fut, PR-eken nem — a napi Linux-alapú munkát nem lassíthatja/blokkolhatja
  egy flaky Windows-futás.
