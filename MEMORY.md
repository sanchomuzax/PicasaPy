...
- **2026-07-22: A KREDIT-/KERET-ÁLLÁS LEKÉRDEZHETŐ — van rá eszköz (fontos: a
  KÉPESSÉG maradjon meg, ne a szám).** Külön session „vakon" van, ha nem tudja,
  hogy egyáltalán meg lehet nézni a Claude-előfizetés terhelését. A mechanizmus:
  1. **Repo:** `sanchomuzax/claude-usage-status` (a felhasználóé). Ha nincs a
     sessionben, `add_repo`-val hozzáadni és klónozni (a jelen session klónja:
     `/workspace/claude-usage-status`). A forrásgép 5 percenként frissíti a
     pillanatképet és felpushol.
  2. **Használat MINDIG két lépés:** előbb `git -C <klón> pull --quiet`, UTÁNA
     `python3 <klón>/budget_check.py --brief` (vagy `--brief` nélkül JSON-ért).
     A pull nélkül elavult számot kapsz.
  3. **A kimenet mindig jelzi a forrást:** `live` = élő; `CACHED Nm old` =
     commitolt pillanatkép (távoli gépen ez a normális). Verdikt + exit-kód:
     `GO` (0) / `CAUTION` (1) / `STOP` (2) / `UNKNOWN` (3).
  4. **Szabályok:** `session unknown (stale)` vagy `UNKNOWN` → előbb `git pull`;
     ha úgy is UNKNOWN, kérj a felhasználótól friss `/usage` számot. A
     `[conserve]`/`[greedy]` címke a felhasználó beállított POLITIKÁJA, nem
     keret-kimerülés. A puszta CAUTION (magas burn-rate) önmagában NEM STOP —
     STOP csak, ha abszolút értékben is elfogyóban a heti keret.
  5. **Mikor nézd:** session elején, subagent-indítás előtt (főleg párhuzamos),
     fázisok között, és ha egy subagent hibával tér vissza (rate-limit ≠ kódhiba
     → előbb keretet nézni). A teljes szabálykönyv: a repó
     `ORCHESTRATOR_PROMPT.md`-je (érdemes elolvasni, ha bizonytalan).
  A LÉNYEG: a következő session tudja, hogy ez a képesség LÉTEZIK és hogyan
  hívható — a konkrét százalék úgyis változik, az nem memória-tétel.
