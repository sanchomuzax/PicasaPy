...
| [ui-lefedettseg.md](ui-lefedettseg.md) | Az eredeti panelek ↔ a mi QML-fánk megfeleltetése · ⭐ **2026-09-06 (#2529): a megszűnt online elemek `nem-cel`-re.** A `ui-lefedettseg-elemek.csv` 191 tétele: **120 `megvan` · 55 `lekutatva` · 17 `nem-cél`**. A tizennégy új `nem-cél` mind a Picasa Web Albums feltöltéshez tartozik (`uploadmgr` 6, `headerpanel` 4, `acquirepanel` 4) — a viselkedésük mérve a `picasa-feltolteskezelo.md`-ben, a döntés helye a `help/features/meg-nem-erheto-el.md` és a #638. ⇒ **feltáratlan 31 → 16 → 11** (a 2026-09-06-i második kör az `edittextpanel` ötösét is lezárta, ld. `szerkeszto-panel-meretek.md` 6.4), nem cél 77 → 91, lefedettség **41,2% → 42,1%**. A maradék 11 elem: `movieeditpanel` 3, `rightdrawerpanel` 2, `acquirepanel` 2, `peoplepanel`/`instructionpanel`/`geopanel`/`gedialog` 1-1 (#2529). ⛔ **128 további elem azért látszik feltáratlannak, mert 85 spec-szakasz HORGONY NÉLKÜL nevezi meg őket** (#2536). *(Előzmény, #2504: hat elavult kézi `hianyzik` felülbírálás javítva — `thumbui/hviewtoggle` `thumbui.tre:406`, `editpanel/movietab` `editpanel.tre:476`, `editpanel/movietabpanel` `editpanel.tre:484`, `editpanel/modaldialogblur` `editpanel.tre:1362`, `editpanel/picnikapply` `editpanel.tre:1430`, `editpanel/preview2` `editpanel.tre:1066`; ettől lett 37 → 31.)* |
