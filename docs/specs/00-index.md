...
### [filterdesc-registry.md](filterdesc-registry.md) — 1 NYITOTT + 1 BLOKKOLT tétel (NYITOTT: a #2125 — az `Invert` csempéjén VAN kék jelvény, pedig `mode="effect"`; a jelvény feltétele mérve `mode==1`, a maradék kérdés a `[0x00d67f68]` szolgáltatás `vtbl+0x14`-e. A `filterdesc.xml`-kérés 2026-09-08-án FELOLDVA: a telepítés-mentésben megvan, bájtra azonos a miénkkel. BLOKKOLT: a #2456, a `PicnikFocalPixelate` HETES alakjának exportja)
