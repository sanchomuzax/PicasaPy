...
### [filterdesc-registry.md](filterdesc-registry.md) — 2 NYITOTT + 1 BLOKKOLT tétel (NYITOTT/1: a #2746 — **miért nem az oktree-út eredménye kerül a képre a `QuantizePalette`-nál, holott a bináris oktree-t épít és a látható kimenet csatornánként egyenletes rácson ül?** A 221. kör két irányt KIZÁRT: a rács nem a `0x00bcb2f0` képponti alkalmazóban keletkezik (nincs benne osztás), és a binárisban nincs név szerinti megkerülő út (egyetlen sztring-hivatkozás, a regisztráló). A 222. kör a `4`-es kódot is KIZÁRTA: 2806 slot-6 hívóhely közül egyetlen sem vizsgálja (három találat, mind hamis pozitív, elolvasva) ⇒ a korai ág élesben nem fut, a FŐ ÚT fut. A 223. kör a munkavégzőt is kizárta (az egyetlen lebegőpontos osztás konstansa 50,0 = a mintaméret), és kiszámolta a keresési kulcsot (`R=r3·32, G=g3·32, B=b2·64`). ⚠️ ÖNHELYESBÍTÉS: a „képfüggetlen" állításhoz NINCS mérés — a három eset ugyanazon kép három BEÁLLÍTÁSA. A megszerzés útja: egy második, eltérő színeloszlású kép exportja (#2770). ⭐ A 224. kör SZÁMMAL szűkítette: a bináris `Steps−1` = 7 színű palettát épít (kiolvasva, `0x00bb5dc4 add eax,-1`), a mért kimenet viszont már a szürke sávon 8, összesen 11 egyedi színt tartalmaz ⇒ a palettaválasztó út a látható kimenetet elvben sem tudja előállítani. A 225. kör megtalálta a jelzők OLVASÓJÁT (`0x008ff550`) és kimérte, hogy a `fullres` egészként, a `slow` logikaiként tárolódik. ⛔ MÓDSZERTANI: az index `string_xrefs` táblája **26 %-ban hiányos** (35-ből 9 ezen a függvényen, köztük a `slow`) ⇒ az indexre alapozott NEGATÍV állítás nem bizonyíték, és ez visszamenőleg gyengíti a 223. kör „egyetlen Quantize-hivatkozás" leletét. A 226. kör INDEXTŐL FÜGGETLENÜL megerősítette, hogy a `.text`-ben pontosan EGY `Quantize`-hivatkozás van (a gyár-regisztráció), és hogy a rövid `QuantizePalette` névre NULLA kódbeli hivatkozás jut ⇒ nincs névre kereső megkerülő út. A 227. kör ezt is lezárta: a `fullres` a leíró `+0xd8` tagjába megy, és a `0x008f6fc0` predikátum a **2**-es értéket vizsgálja — a leíróban viszont `fullres="2"` **egyetlen** szűrőn áll (a `FocalZoom`-on), a `QuantizePalette` `fullres="1"` ⇒ a „külön renderút" hipotézis ezen a szálon ELESIK. ⛔ **A 228. kör kimondta: a GÉPI LÁNC KIMERÜLT** — nyolc irány kizárva, a rács a szűrő-láncban sehol nem keletkezik. A jegy **BLOKKOLT**, a #2770-es mérésre vár; addig gépi kört nyitni nem érdemes. NYITOTT/2: a #2125 — az `Invert` csempéjén VAN kék jelvény, pedig `mode="effect"`; a jelvény feltétele mérve `mode==1`, a maradék kérdés a `[0x00d67f68]` szolgáltatás `vtbl+0x14`-e. A `filterdesc.xml`-kérés 2026-09-08-án FELOLDVA: a telepítés-mentésben megvan, bájtra azonos a miénkkel. BLOKKOLT: a #2456, a `PicnikFocalPixelate` HETES alakjának exportja)

⭐ **2026-09-05 — a Comicize pontmaszkja STATIKUS: a tónus nem a maszkból jön (#2476).** A `filterdesc.xml` két `TiledImageMask`-ja (`_mskColorSpots1/2`) **semmilyen képfüggő attribútumot nem kap** — a tónus a láncból jön (`AdjustCurves` → `Pixelate` → `BW` → küszöbgörbe → `GetVar … Mask`). ⚠️ **Nálunk fordítva:** az `apply_comicize` a `halftone_branch`-et hívja, ami a pont SUGARÁT modulálja a tónussal (`halftone.py:82`), miközben a `TiledImageMask` tényleges megfelelője (`tiled_dot_mask`, `halftone.py:94`) **nincs használatban** ezen az úton. **A rajzoló primitív kimérve:** a csempe-előállító (`0x00bbaa90`) ugyanazt a párost hívja (`0x008f3840` + `0x008f3970`), mint a `CircularGradientImageMask` (`0x00bc2a50`), **ugyanazzal a harmadik argumentummal (2)** — és az nem alakzat, hanem **megállószám** (a `0x008f3970` bájttömbként olvassa, `[ebp+0xc]−1` hosszú cikluson; a harmadik hívó 15-öt ad). ⇒ kétmegállós színátmenet, nem kemény küszöb. **MÉRVE** (fázis-hajtogatás a 24-es csemperácsra, `comicize-sweep/…/Comicize_050.jpg` + a Picasa exportja): a referencia profilja **fennsíkos** (1,00/0,93/0,82), majd `r=4`→0,40 és `r=6`-ra nulla; a miénk fennsík nélkül monoton lejt, `r≈7`-ig elhúzódó farokkal. Csúcs-csúcs **13,20 vs 25,89 (1,96×)**, csempénkénti amplitúdó **26,2 vs 36,4 (1,39×)** — ez erősíti meg a #1606 „kb. 1,5×"-ét. Jegy: **#2476** (`ready`, a javítás fejlesztés).
