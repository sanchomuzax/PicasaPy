...
⛔ **HELYESBÍTVE 2026-09-16 (#2456): a `merokit-2` szett TARTALMAZTA a hetes alakot.** A szett saját `.picasa.ini`-jében ott áll a `PicnikFocalPixelate=1,0.500000,0.500000,40.000000,60.000000,50.000000,0.000000;` bejegyzés — vagyis `1` + fókusz(x, y) + Impact + Radius + Hardness + Fade, a TELJES alak. A #1142 mérése szerint az eredeti erre is a forrást adta vissza (0,164 = JPEG-zaj), miközben a mi renderelőnk 29,19-es eltérést okozott ⇒ a `chain.MEASURED_NOT_RUNNING_OPS` besorolás **megalapozott**, a render-kezelő bekötése mérhetően rontana. Az alábbi bekezdés ezt tévesen állította az ellenkezőjét; a „9 rövid alakból 9 tétlen” érvelés önmagában áll, de erre a szűrőre nem alkalmazható. A korábbi megmaradó kérdés **LEZÁRVA (2026-09-18)**: a csempe-kattintó út statikusan kiolvasható. A `0x005d59f0` azonosítja a Shift-párt, a közös végpont `0x006021d0` → `0x005f8520` pedig az élő effekt-alkalmazási útba viszi; a részletes bizonyíték a `filters-decoded.md` és az `ui-audit-editor.md` #2456-szakaszában van. A megvalósítás külön fejlesztői jegy: **#3315**.
