...
⭐ **2026-09-18 — #710 forrásból megválaszolva, a jegy nyitva marad a terméki átvezetésre.** Az `editpanel.tre` a négy `editsliderN_container` elemet (N=1..4) egyetlen közös `editcontrol_well` alá teszi; a `tab3`–`tab5` nem vezet be effektfüggő slider-konténert. Az index `string_xrefs` táblájában a 2–4. konténer közös feldolgozó címe `0x007518e0`, külön effektcsúszka-családnak nincs nyoma. A már rögzített rétegtéglalap-mérés mind a négyre **191 × 27 px**, x=**30..221**; a **127 × 27 px** a külön `scaleslider`/Derítőfény-család. A kutatási válasz bekerült a lap 4.3/a szakaszába; a PicasaPy paraméter-alpanelének fix szélességre igazítása fejlesztői teendő, a #710-et ezért nem zárjuk le.
...
⭐ **2026-09-07 (180. kör) — A PÁSZTÁZÓINK VAKFOLTJA MÉRVE ÉS LEZÁRVA; a `.cxf`-beolvasó TELJES mezőtérképe (#1412, `kollazs-eletciklus.md` **17.15–17.16**).** ⛔ **A 17.7 szűrője hiányos volt:** csak `fstp`/`fst`-re nézett, holott az MSVC a float-ot egész regiszteren át is tárolja (`fstp [esp+N]` → `mov r,[esp+N]` → `mov [obj+N], r`) — **a bizonyíték magában a 17.7 által idézett `FUN_00885060`-ban van** (`0x0088520f`: az `x`-et `mov`-val írja). ⭐ **Négy pásztázás, mindegyik POZITÍV KONTROLLAL:** (1) tömb-alakú (`bázis+index+0x2c`) írás az EGÉSZ binárisban = **8**, ebből a két téma-elrendező (`0x0088522d`, `0x008885bc`, mindkettő `fld1` ⇒ **1,0**), a többi hat tételesen kizárva (szűrő-csúszka felülete, `0x30`-as lépésköz, futásidejű tábla, foglaló-rekesz); (2) mutatós `mov [reg+0x2c], <float>` a teljes `.text`-ben = **9**, a kollázs-sávbeli **négy mind ugyanaz a `free`-és-újraköt idióma** (`call 0xc07738`) ⇒ a `+0x2c` ott **tárolómutató**, nem méretarány; (3) a **313,0 lebegőpontos bitmintája a TELJES fájlban: `float32` 0 · `double` 0** találat ⇒ a beégetett-konstans hipotézis véglegesen elvetve (a 17.4 csak az egész immediate-et nézte, csak a `.text`-ben); (4) a `"scale"` sztringre az egész programban **pontosan két** hivatkozó: a beolvasó (`0x00832830`) és a kiíró (`0x008347b0`). ⇒ **A 17.14 (b) ága — „van egy nem látott író" — a tömb-alakú és a mutatós `mov`-os utakon LEZÁRVA, negatívval;** marad a `lea r,[r+0x2c]` út. Az **(a)** ág (a `scale` a fájlból öröklődik) az egyetlen, amit a bináris nem cáfol — a döntő mérés változatlanul egy FRISSEN létrehozott kollázs `.cxf`-je. ⭐ **ÚJ: a `.cxf`-beolvasó teljes attribútum→mező térképe** (dokumentum-szint: `version`, `format` négy int, `orientation` `[+0x20]` 1/0, `albumID` `[+0x30]`, `background type` `[+0x24]`, `color` `[+0x28]` alap `0xFF000000`, `spacing` `[+0x40]`; csomópont: `x/y/w/h/theta/scale` = `[ebx+0x54…0x68]`) **és az ALAPÉRTELMEZÉSEK**: `scale` **1,0** (`0x00832fbd fld1`), `theta` **0,0** (`0x00832fa2 fldz`) ⇒ hiányzó `scale`-attribútum esetén az érték `1,0` — a PicasaPy olvasójára közvetlenül átvehető. ⚠️ **Kimondott, feloldatlan belső ellentmondás:** a 17.11 szerint a `[ebx+0x48]` bájteltolás, a `0x00834c2d` blokk szerint tömb-bázis; a verem-egyenleg elágazásokon át **lineárisan nem számolható**, ezért NEM döntöttem el — dekompilátoros kör kell hozzá.

⭐ **2026-09-07 (179. kör) — A LÁTSZÓLAGOS ELLENTMONDÁS FELOLDVA: az elrendező IDEIGLENES vektorba ír, nem a mentett tömbbe (#1412, `kollazs-eletciklus.md` **28.**).** ⭐ **Codespace NÉLKÜL, helyi capstone-pásztázással.** ⛔ **A 22. kör óta nyitva álló ellentmondás — „az elrendező `1,0`-t ír, a fájlban mégis `313` áll, tehát valami FELÜLÍRJA" — a KÖVETKEZTETÉSÉBEN volt hibás:** nem felülírás történik, hanem **két külön tömbről** van szó. ⭐ **Bizonyíték utasításszinten:** a `.cxf`-író a DOKUMENTUM `+0x48` mezőjéből veszi a tömb bázisát (`0x00834c2d mov edx,[ebx+0x48]` → `0x00834c3f fld [eax+edx+0x18]`, majd `+0x1c`, `+0x20`, `+0x24` ugyanígy) — **eltolás-delta nincs**, tehát a 26.1 mezőtérkép címzés-szinten is igazolt; az elrendező hívója viszont **veremlokálist** ad át és a hívás után **elpusztítja**: `0x00887b93 lea ecx,[esp+0x18]` · `0x00887b98 push esi` · `0x00887b99 call 0x888210` · `0x00887b9e lea esi,[esp+8]` · `0x00887ba4 call 0x62d010` (a vektor pusztítója; ugyanez a korábbi ágon `0x00887b48`/`0x00887b4c`). ⇒ **az elrendező `1,0`-ja megsemmisül a függvényből kilépve.** ⭐ **A 19. kör szerkezeti sejtése („van egy visszamásolás") ezzel IGAZOLT**, és a 26.4/27. negatívja is érthetővé válik: a keresés a rossz tömbre irányult. ⭐ **Három pásztázás, kimondott kontrollal:** a `+0x48` OLVASÓI a sávban (`esp`/`ebp` bázis kizárva) **106** függvény — **a pozitív kontroll (`FUN_008347b0`) MEGVAN**, tehát a pásztázó helyes; az ÍRÓK **47**; a `+0x48`-at ÉS a `+0x38`/`+0x3c`/`+0x40` valamelyikét együtt írók (**dokumentum-alak**) **16**, ebből `dword`-ként írók — a `.cxf`-író így olvassa — tízen: `0x00829d60` · `0x00829dc0` · `0x0082a250` · `0x00832500` · `0x00838ef0` · `0x0084bb30` · `0x0085ff90` · `0x00865890` · `0x008833b0` · `0x0088b0a0`. ⭐ **A KÖVETKEZŐ lépés most pontosabb, mint eddig bármikor:** *ki tölti fel a dokumentum `[+0x48]` tömbjét, és a `+0x2c`-t ott honnan veszi?* — (1) a tíz `dword`-íróból a kollázs-dokumentum feltöltője (kiemelt jelölt a `0x00832500`, mert a `+0x3c`-be nem nullát, hanem **2**-t ír ⇒ verzió/típus mező); (2) a `collagepanel/cnode_` elemek visszaolvasása (a `FUN_00888b40` elemkészítő maga is ír `[esi+0x48]`-at, `0x00888ce2`); (3) a csomópont értékadó operátorának (`FUN_008341b0`) **tizenkét hívója** közül az, amelyik a dokumentum tömbjébe másol. Jegy: **#1412** (nyitva marad).

⭐ **2026-09-07 (178. kör) — ⛔ ÖNHELYESBÍTÉS: a 177. kör „kimerítő negatívja" VAK VOLT az index-alakra (#1412, `kollazs-eletciklus.md` **27.**).** ⛔ **A 26.4 állítása — „a kollázs-sáv 878 függvényéből egyik sem ír float-ot a csomópont `+0x2c`-jébe" — MEGDŐLT.** Az ellenpélda a sávon BELÜL van: a csomópont értékadó operátora, `FUN_008341b0` (`0x008341b0`, 252 b), `0x00834261 fld [esi+0x2c]` → `0x00834264 fstp [ebx+0x2c]`. A 177. kör listája nem tartalmazza, és nem dekompilációs hiba miatt (a kimenetben a `008341b0` **nulla** előfordulás, sikertelen dekompiláció **0**). ⭐ **A mechanizmus:** a pásztázó csak a BÁJTELTOLÁSOS alakot ismerte (`+ 0x2c`), a Ghidra viszont a mezőmásolást **INDEX-alakban** írja ki — `0x2c/4 = 11` —, szó szerint `param_1[0xb] = unaff_ESI[0xb];`. ⛔ **A gyökér módszertani:** a pásztázást **nem ellenőriztem ismert pozitívval**, pedig a 19. kör óta tudjuk, hogy épp ez a függvény másolja a `+0x2c`-t. ⭐ **A JAVÍTOTT pásztázás (három alak: BAJT · INDEX · MEZO, kötelező pozitív kontrollal) ÉRVÉNYES:** 878 függvény, 0 dekompilációs hiba, **BAJT 37 · INDEX 37 · MEZO 0** írás, a kontroll megvan ⇒ **a keresés fele hiányzott**. A 26.4 negatívja visszavonva; ami áll belőle: a bájteltolásos alakban nincs számolt `scale`-író a sávban. ⚠️ **ÚJ MÓDSZERTANI KIKÖTÉS:** a `[0xb]` csak akkor `+0x2c`, ha a mutató a tárgy BÁZISA — ellenpélda a `FUN_008921a0`, ahol `puVar17 = (undefined4 *)(iStack_84 + 0x10)`, tehát a `puVar17[0xb]` valójában `+0x3c`. A 37 INDEX-találat **jelölt, nem bizonyíték**. ⛔ **AMIT KIZÁRTAM:** a legígéretesebb tétel, a `FUN_008378c0` (`0x008378c0`) `+0xf8 → +0x2c` másolása „piszkos" jelzővel (`param_2[0xb] = param_3[0x3e]`, mellette `+0x24 := +0xec` és `+0x28 := +0xf0` — a csomópont `h`/`theta`/`scale` sorrendjével EGYEZŐ mezősor) **nem geometria**: a forrásmezőt a `FUN_00836510` (`0x00836510`) egy **gyorsítótár-kulcs hash**-sel tölti (mag `0x12345678`, `"%d:%d:%d:%d"` kulcs), és mindkét objektum ugyanazt a zárolási idiómát viseli (`[8]` szálazonosító, `[9]` rekurziószám, `+0x28` `CRITICAL_SECTION`) ⇒ a **bélyegkép-gyorsítótár** osztálya (RTTI: `CCollageBitmapProvider`), nem a kollázs-csomópont. A mezőegyezés véletlen. ⭐ **A KÖVETKEZŐ lépés megnevezve:** a 37 INDEX-találat **bázisának** egyenkénti tisztázása, és azok kiszűrése, amelyek nem 56 bájtos csomópontra mutatnak; a `FUN_008341b0` · `FUN_00835520` · `FUN_00838d10` · `FUN_0084c7b0` **másol**, tehát a forrás felé kell menni. Jegy: **#1412** (nyitva marad).

⭐ **2026-09-07 (177. kör) — a felhős dekompiláció LEFUTOTT: a K2 LEZÁRVA, a K1 pedig KIMERÍTŐ negatívot kapott (#1412, `kollazs-eletciklus.md` **26.**).** ⭐ **A blokkoló elhárult:** a token hatóköre most `codespace, gist, read:org, repo, user, workflow` (`gh api user/codespaces` → 200), a `picasapy-agent` #53 lezárva. Ghidra **12.1.2**, `Picasa3.exe` SHA-256 `644b7bec…dc96`. ⭐ **A `.cxf` MEZŐTÉRKÉP most már a KIÍRÓ oldaláról bizonyított:** a `FUN_008347b0` (`0x008347b0`) a hat lebegőpontos attribútumot ebben a sorrendben olvassa: `+0x18` = `x`, `+0x1c` = `y`, `+0x20` = `w`, `+0x24` = `h`, `+0x28` = `theta`, **`+0x2c` = `scale`** — mindig `param_2 + <eltolás> + *(int *)(param_1 + 0x48)` címzéssel, és a függvény sztringkészletében ott a `theta` és a `scale` attribútumnév. Eddig ez a térkép a FÁJL elrendezéséből jött. ⭐ **K2 LEZÁRVA:** a `FUN_009b4aa0` teljes C-kódja megvan — `z = min((célSzél+0,499)/forrSzél ; (célMag+0,499)/forrMag)`, kimenet `(0, 0, KEREK(z·forrSzél + 1e-5), KEREK(z·forrMag + 1e-5))` ⇒ **KEREKÍT, nem csonkol**; a két konstans a binárisból, most kiolvasva, mindkettő `double`: `0x00cf4160` = **0,499** (fájloffszet `0x8f4160`), `0x00cf41e0` = **1e-05** (`0x8f41e0`). A regiszter-szerepek a hívás helyén: `0x00888438 lea eax,[esp+0x88]` = a **méretezendő**, `0x0088843f lea esi,[esp+0x78]` = a **célkeret**, `0x00888443 lea ecx,[esp+0x60]` = a kimenet. ⛔ **ÖNHELYESBÍTÉS a 24.2-höz:** a „csak ha nem nulla, tölti a doboz W/H-ját" olvasat **fordított volt** — a `0x0088840b je 0x888423` szerint a **cellaméret a TARTALÉK ág** (a virtuális hívás nem nulla visszatérése = hiba), a rendes úton a `[esp+0xf8]`/`[esp+0xfc]` azt tartja, amit a hívott írt bele (`&[esp+0x98]` + `0x60` = `0xf8`) ⇒ **az `EAX`-rect a FORRÁSKÉP mérete**, és a 25.4 ellentmondása a **mérés** javára dől el. ⭐ **K1 — KIMERÍTŐ NEGATÍV, dekompilátor-szinten, három független pásztázással:** az interaktív képhozzáadás fája (`0x0082a670`, 2 szint, **214** függvény) → **nulla** valódi `+0x2c`-írás; a mentési ág (`0x00834700`, 3 szint, **69** függvény) → **0 ÍRÓ · 1 OLVASÓ**; a teljes kollázs-sáv (`0x00829000`–`0x00895000`, **878** függvény) → 24 `+0x2c`-író, de **egyik sem** csomópont-`scale`: két literál `0x3f800000` (=1,0f, `FUN_00885060` és `FUN_00888210`), a többi egész/mutató/jelzőbit. A három „`+0x2c := +0x40`" másolás (`0083d730`, `0087b4a0`, `0088ac30`) a panel `[+0x270]` objektumának **állapot-mentése**, nem csomópont (a csomópont lépésköze `0x38`, a `+0x40` kívül esik). ⇒ **a `scale` írója a kollázs-sávon KÍVÜL van.** ⛳ **A hatókör számokkal:** 20 608 függvényből 878 (a program **4,3%-a**) van kizárva; a sáv dekompilálása ≈20 perc, a teljes program ≈**8 óra** ⇒ egy körbe nem fér. **A megnevezett következő vágás:** (1) a kollázspanel parancs-elosztójának (`0x0082d570`) fája 3 szint mélyen — ide tartozik a kézi átméretezés (17.5); (2) a téma-osztályok metódusai (RTTI: `CContactSheetTheme`, `CPileTheme`, `CGridTheme`, `CFrameGridTheme`, `CMultiExposureTheme`); (3) csak ezután a teljes program, címtartományonként. ⭐ **Nálunk (MÉRVE):** a `collage/fitting.py:60–67` `fit_inside()`-ja **bitre ugyanezt** számolja (`0.499`, `1e-5`, `floor(x+0,5)`) ⇒ **nincs teendő**; az érték az, hogy a képlet mostantól dekompilátumból igazolt, nem mintából illesztett. Jegy: **#1412** (nyitva marad), `picasapy-agent` **#53** (lezárva).

⭐ **2026-09-07 (176. kör) — a LEVÉLNÉV-lelet MA 177 (nem 26), a jegy haszna viszont MEGSZŰNT: a feltáratlan oszlop NULLA (#2530; kilenc spec-lap).** ⛔ **A jegy premisszája ELAVULT, mérve.** A #2530 törzse (2026-09-06) 160 levélnév-leletet és ebből **26 „ma is a hiánylistán álló"** elemet ír, azzal az indoklással, hogy a javítás „a lefedettségi számot is mozgatja". A mai mérés tiszta main-munkafában: a levélnév-figyelmeztetés **177 elem**, a `ui-lefedettseg.md` **feltáratlan oszlopa viszont 0** (a #2529/#2536 körei elvitték) ⇒ **a javítás a lefedettségi számot NEM mozdítja**; a haszna higiénia és regresszió-védelem, nem szám. ⭐ **67 elem javítva, 0 regresszió** (177 → **110**, két teljes mérés különbsége): `collagepanel` 17 · `publish` 15 · `acquirepanel` 14 · `foldermgr` 10 · `initialscan` 5 · `headerpanel` 3 · `thumbui`/`makemoviepanel`/`geopanel` 1-1. ⭐ **MÓDSZERTANI LELET: a figyelmeztetés „hol" oszlopa az ELSŐ ábécésorrendű horgonyzott szakaszt nevezi meg, nem feltétlenül a helyeset.** Bizonyíték: az `initialscan/radiogroup`-ot az `export-parbeszed.md: 0x00738c88`-ra tette, ahol a `radiogroup` az export-párbeszéd **XML-elemtípusa** (`<radiogroup name="sizeradio">`) — a valódi szakasz a `picasa-elso-inditas.md` 2. ⇒ **a „hol" oszlop jelölt, nem cím.** ⛔ **Négy HAMIS jelölt, névvel:** `printpanel/previewlabel` (az `picasa-importalas.md` „Preview" sora a `acquirepanel_text.tre` felirata; `acquirepanel/previewlabel` a leltárban NINCS — ahogy `acquirepanel/importtraylabel` sem); `initialscan/radiogroup` a fenti helyen; `activitycontainer` (három panel alatt is van, ezért minősítetlen maradt); `thumbui/editpanel` (a `00-index.md` szakasza az `editpanel` PANELRŐL szól). ⭐ **Két szó szerinti idézetnél nem a szöveget írtuk át, hanem MELLÉ tettük a teljes névsort** — `picasa-mappakezelo.md` 1.1 (a `tre:foldermgr` csomópontfája, 21 név) és `picasa-elso-inditas.md` 2. (a tervezővászon táblája, 17 név) —, mert a „szó szerint" állítást a minősítés megtörné. Ugyanez a `picasa-arcfelismeres.md` 15.2-nél: az elosztó (`0x005e0f70`) `repe cmpsb`-vel a **puszta** nevet hasonlítja, és a parancs a három fejléc-változat bármelyikéből jöhet ⇒ a tábla marad levélnevű, a teljes névsor alatta áll. ⛳ **A maradék 110 elem panelenként:** `editpanel` 23 · `collagepanel` 19 · `thumbui` 15 · `printpanel` 14 · `makemoviepanel`/`geopanel` 6-6 · `peoplepanel` 5 · a többi 1–3. Jegy: **#2530** (nyitva marad).

⭐ **2026-09-07 (175. kör) — a CÉLKERET azonosítva, a forrásrect NEM, és a drága út JOGOSULTSÁGON akadt el (#1412, `kollazs-eletciklus.md` **25.**).** ⛔ **A felhős Ghidra-kör NEM indítható — mérve:** `codespace_re.py doctor` → `HTTP 403 … This API operation needs the "codespace" scope`; a session-token hatóköre `gist, read:org, repo, workflow`, a bot-token nem alkalmas, **helyi Ghidra nincs**. ⚠️ **A költségkeret NEM akadály** (`GO · session 25% · weekly 60% · burn 0,64x`) ⇒ **az akadály jogosultság**. Üzemeltetési jegy: `picasapy-agent` **#53** (`felhasználóra-vár`). ⭐ **A `FUN_00888210` teljes verem-rekesz-nyilvántartása** (capstone, minden `[esp+X]` írás) eldöntötte a 24.3 kétértelműségének **egyik felét**: a `FUN_009b4aa0`-hívás (`0x0088844e`) **`ESI`-rectje = (0, 0, cellaSzél, cellaMag) = a CELLA** — a `[esp+0x80]` a `[esp+0x2c]`-ből, a `[esp+0x84]` a `[esp+0x44]`-ből jön, és azok a 18.2 cellaméret-`fistp`-jeinek eredményei (`0x0088833f`, `0x0088835c`, `0x0088839b`, `0x0088839f`). Az `ESI` a **célkeret**. ⚠️ **A forrásrect NEM dőlt el, és a helyi módszer korlátja ki van mondva:** az `EAX`-rect mérete a `[esp+0xf8]` / `[esp+0xfc]`-ből jön, amit a pásztázás szerint csak a nem-nulla ág ír — **de a pásztázás nyers `esp`-eltolásra illeszt és nem követi az `esp` mozgását** (`sub esp,0x124` + négy `push`), tehát más veremállapotban írt érték kimarad. ⇒ **a 174. kör ítélete áll: ez dekompilátor-munka.** ⛔ **KIMONDOTT ELLENTMONDÁS (25.4):** ha a célkeret a cella, egy álló kép magassága `cellaMag` volna (lap-szintű állandó), a mérés viszont az `AI27`-en három különbözőt ad (446,08 · 449,75 · 432,00). Ellenőrizhető számpélda: cella 450×571, kép 816×1456 ⇒ `min(zx,zy) = zy = 0,39251` ⇒ kimenet `(320, 571)`, rés levonva `(248, 499)`; **mért: `(250, 446,08)`**. A két lehetőség (a forrásrect nem a kép mérete · a tárolt `w`/`h` nem ennek a kimenete) ugyanazzal dől el. Jegy: **#1412**, `picasapy-agent` **#53**.

⭐ **2026-09-07 (174. kör) — K2: `k = min(cellaSzél, cellaMag)`, és az Indexkép-illesztés KÉTÉRTELMŰSÉGE (#1412, `kollazs-eletciklus.md` **24.**).** ⛔ **A kör NEM illesztett számot a négy pontra** (313 · 500 · 256 · 158) — a projekt szabálya tiltja, a képletnek a kódból kell jönnie. ⭐ **Új, ellenőrzött összefüggés:** `k = min(cellaSzél, cellaMag)` **4/4** a mintákon (300 · 450 · 300 · 186), és van rá szerkezeti indoklás is: az `oszlop = ⌊W'/k⌋`, `sor = ⌊H'/k⌋` egészosztásból mindkét cella legalább `k`. ⚠️ **Általános bizonyítás nincs** — négy mintán igaz. ⭐ **Az illesztés magja utasításszinten** (`0x008883cd`–`0x0088846f`): a csomópont+8-ból kulcsot épít (`0x00835380`), **virtuális hívást** tesz a panel `[+0x270]` objektumára, és **csak ha az nem nulla**, tölti a doboz W/H-ját (`[esp+0x2c]` / `[esp+0x44]`); majd `FUN_009b4aa0` arány-tartó illesztés (`0x0088844e`) és a kimenet **beszűkítése rés-sel mind a négy élen** (`0x00888463`–`0x0088846f`). ⇒ a tárolt `w`/`h` = **illesztett doboz − 2 × rés**, most már utasításszinten. ⛔ **KÉTÉRTELMŰSÉG, amit a kör NEM dönt el:** a `FUN_009b4aa0` `EAX`-rectje a méretezendő (a kimenet ennek az arányát tartja). A **mérés** szerint a tárolt doboz a **forráskép** arányát viszi (10⁻³ egység alatt, 18.3) ⇒ `EAX` = a kép; a **regiszter-hozzárendelés** viszont a számított egész párt (`[esp+0x2c]`/`[esp+0x44]`) teszi az `EAX`-rectbe ⇒ cellaméret. A feloldáshoz a 2337 bájtos törzs **verem-nyilvántartása** kell — az már **dekompilátor-munka**. ⇒ **A K2 olcsó lánca is a DRÁGA úthoz ér, a K1-gyel EGY menetben:** egyetlen Ghidra-futás két gyökérrel — `0x0082a670` (kollázspanel, K1) és `0x00888210` (Indexkép-elrendező, K2). Jegy: **#1412**.

⭐ **2026-09-07 (173. kör) — K1: az OLCSÓ lánc KIMERÜLT, és egy SÁV-DEFINÍCIÓS hiba a saját méréseinkben (#1412, `kollazs-eletciklus.md` **23.**).** ⛔ **ÖNHELYESBÍTÉS:** a 169. és a 172. kör „kollázs-sávja" (`0x00820000`–`0x008fffff`) **túl tág volt, ellenőrzés nélkül** — az alja (`0x00820000`–`0x00826000`) **arcfelismerés** (`SmartMultiPersonTrans`, `conf(%f),pan(%f),leye(…)`). A valódi kollázs-kód `0x00829…`-tól kezdődik. A negatívokat ez **nem gyengíti** (tágabb sávban több jelöltet néztünk át), de a **számok felfújtak voltak**; a jövőbeli pásztázások a `0x00829000`–`0x00895000` tartományt használják. ⭐ **A `FUN_0087c470` NEM ír vissza** — utasításszinten: `0x0087c510 fld [ebx+0x2c]` → `0x0087c513 fstp [esp+0x38]` (LOKÁLISBA), és a tartalék (`param_5`) is csak a lokálist cseréli (`0x0087c536`); a csomópont mezője érintetlen. ⭐ **Futásidőben számolt eltolású FLOAT tárolás** (`fstp [bázis+index]`, nulla eltolás): a teljes `.text`-en **66**, a valódi tartományban **8** — ebből 5 `*4`-es (float tömb), a maradék három elolvasva: egy tömbindexelés (`0x008734bd`), kettő **polárkoordináta-átváltás** (`FUN_0088c480`: `sqrt` és arkusz tangens egy közös bázisra, `0x0088cc70`/`0x0088cc95`). **Egyik sem a csomópont `scale`-je.** ⭐ **Blokk-másolás: NINCS** — `mov ecx,0xe` + `rep movsd` (56 bájt) **15 találat, mind az ARCFELISMERÉSBEN**, a kollázsban nulla; a `push 0x38` + `call` egyetlen kollázs-találata a már ismert `push_back` foglalása (`0x00833ac3`). ⇒ **A K1 olcsó bizonyítéklánca (index → sztring/xref → helyi pásztázás → meglévő dekompilátum) KIMERÜLT** — a kizárt alakok teljes listája a 23.5-ben. **A következő lépés a DRÁGA út, megnevezve:** célzott Ghidra-dekompiláció a kollázspanel **képhozzáadási** ágára (`0x0082a670` hívási fája két szint mélyen) azzal a kérdéssel, hol kapja a frissen felvett csomópont a `+0x2c`-t. Jegy: **#1412**.

⭐ **2026-09-07 (172. kör) — K1: a `lea`-út KIMERÜLT (pozitív kontrollal), és a keresés iránya MEGFORDULT (#1412, `kollazs-eletciklus.md` **22.**).** ⭐ **A `lea`-alakú közvetett írás** — a 19.2 kimondott hatókör-hiánya — végigpásztázva: **207** `lea …+0x2c` címképzés a teljes `.text`-en, ebből **62**-ben van tárolás a kapott mutatóra, a kollázs-sávban **3**. A három: a már ismert betöltési szorzás (`0x0083468a`, ⛳ **pozitív kontroll** — a pásztázás megtalálta), és két **hivatkozásszámlált sztring**-idióma (`0x008300dc`, `0x00860032`), nem csomópont. ⇒ **érvényes negatív**, nem a minta hibája. ⭐ **Utasításszinten igazolva:** mindkét elrendező **feltétel nélkül `1,0`**-t ír a csomópont `+0x2c`-jébe — `contactsheet` `0x008885ac fld1` → `0x008885bc fstp [ebx+eax+0x2c]`, `regulargrid` `0x0088520d fld1` → `0x0088522d fstp [eax+esi+0x2c]`; és **függvényenként pontosan EGY** ilyen tárolás van (a többi `[esp+0x2c]` lokális). ⭐ **A mentés-szervező nem alakít át:** `FUN_00834700` (174 b) csak puffert épít, meghívja az XML-írót és a fájlba írót. ⭐ **A csomópont-hozzáadó VIRTUÁLIS metódus:** a `FUN_00833920`-ra **nulla** közvetlen `call` van, és a cím a fájlban **egyetlen** helyen áll adatként — `0x00cbf898`, a `CCollageParser` vtáblájának 8. slotja (`0x00cbf878`). ⇒ **a staging → `push_back` út a FÁJLBEOLVASÁSÉ**, nem az interaktív képhozzáadásé. ⭐ **Ezzel a keresés iránya megfordul:** egy ÚJ kollázsnál sem a téma-elrendező (csak `1,0`), sem a beolvasó nem adja a `scale`-t ⇒ **a hozzáadási úton kell keresni** (`collagepanel/`), nem a téma- és nem a parser-kódban. ⛔ **A `+0x2c`-írás kizárt alakjainak TELJES listája** most a 22.6-ban áll (öt címzési alak + a kupac-fa + a mentés-szervező); **ami maradt:** (1) futásidőben számolt eltolás (`[reg+reg]`, a `0x2c` regiszterben — tulajdonság-beállító), (2) blokk-másolás egész csomópontra, (3) a sávon kívüli kód. Jegy: **#1412**.

⭐ **2026-09-06 (171. kör) — a `scale` MÉRTÉKEGYSÉGE bizonyítva, és a 20.1 mechanizmus-olvasata HELYESBÍTVE (#1412, `kollazs-eletciklus.md` **21.**).** ⛔ **ÖNHELYESBÍTÉS:** a `picturepile` per-csomópont létráját **nem** a `0x0082c9a0` állítja elő, hanem a **kupac-elrendező `FUN_0087bcb0`** (`0x0087bcb0`), és ott a szorzó a **lap szélessége**, nem a beégetett `1024,0`. A képlet és a három konstans helyes, a mérés (55/57) áll — a HELY és a SZORZÓ volt téves. **Miért egyezett mégis a szám:** a `.cxf` lapszélessége épp 1024 egység (18.3), így `lapszélesség × 0,33` = `1024 × 0,33`. ⚠️ Ez pontosan a „a megfejtett mechanizmus nem diagnosztizált ok" csapdája — egy körrel azután, hogy a 19.2 hatóköre helyesen ki lett mondva. ⛳ **A per-INDEX olvasat viszont most már KÓDBÓL is igazolt:** a `local_14` az elrendező ciklusváltozója, 1-től indul. ⭐ **A `scale` MÉRTÉKEGYSÉGE a FOGYASZTÓ oldaláról:** a `FUN_0087c470` (`0x0087c470`) az elemre `csomópont.scale × (lapszélesség / 1024)` nagyítást alkalmaz — a konstans **`0,0009765625` = 1/1024** pontosan. ⇒ **a `scale` hossz, a lap szélességének 1024-ed részeiben**; a 20.5 egységes olvasata ezzel az író ÉS a fogyasztó oldaláról is áll. ⭐ **A tárolt `scale` ELSŐBBSÉGET élvez:** az elrendező számolta méret csak akkor lép be, ha a csomópont `scale`-je **nulla** — ez magyarázza a 17.5 kézi átméretezéseit is. ⭐ **ASZIMMETRIA:** az Indexkép elem-létrehozója (`FUN_00888b40`) **csak a pozíciót** állítja be — se forgatás (`FUN_009dec60`), se nagyítás (`FUN_009deca0`), szemben a kupacéval, amely mind a hármat. ⇒ **az Indexképnél a `scale` a rajzolásba egyáltalán nem megy bele**, tisztán elrendezési mennyiség (18.5) — ezért nem volt levezethető a rajzolt dobozból. ⛔ **KIZÁRVA:** a `FUN_0087cb70` és a `FUN_0087c470` **sem ír** a `+0x2c`-be ⇒ **a kupac-fában sincs `+0x2c`-író**; abból, hogy a fájl `S(i)`-t tartalmaz, NEM következik, hogy a kupac-elrendező írta volna. **Ki írja a `+0x2c`-t — mindkét témára — továbbra is nyitott.** Melléklelet: a polaroid felirat (`FUN_0087c820`) `max(szél, mag)/1024` egységet használ, nem a szélességet. Jegy: **#1412**, **#2583**.

⭐ **2026-09-06 (170. kör) — a `picturepile` `scale` képlete BINÁRISBÓL, és a `scale` egységes olvasata (#1412, `kollazs-eletciklus.md` **20.**).** ⭐ **A képlet és mind a három konstansa kiolvasva** (`FUN_0082c9a0`, `0x0082c9a0`): `f = 1/sqrt(sqrt(n)−1)` felülről 1,0-ra korlátozva (`0x0082ca29` · `0x0082ca2e` · `0x0082ca3f` · `0x0082ca4b` · `0x0082ca57`), majd `S = CSONK(f × 1024,0 × 0,33)` — konstansok: **1,0** (`0x00c7e328`), **1024,0** (`0x00cf4218`), **0,33** (`0x00cf46c0`); az eredmény a téma `+0x3c` mezőjébe megy (`0x0082cad0`). ⭐ **A CSONKOLÁS is a binárisból:** a `0x0082cac4` `fldcw` a `0xc00` módot állítja a `fistp` elé — a **#1059** ezt kilenc mintából vezette le („9/9 `floor`, 1/9 kerekítés"), most bináris megerősítést kapott. ⚠️ **Ez NEM új szabály a kódunkban:** a `collage/pile.py` `pile_size()` pontosan ezt számolja (`PILE_BASE_RATIO = 0,33`, csonkolással) — ami új, az a `mérés (9 minta)` → `bináris (0x…)` váltás. **Mérés 55/57**, és a két kivétel az `AI2` két **kézzel átméretezett** csomópontja — a 17.5 független leletével kölcsönösen igazolva. ⭐ **A létra INDEX szerinti:** az `AI10` öt csomópontja `337·337·337·337·303`; darabszám-alapú képlettel mind az öt 303 volna. ⛔ **Amit a `Scale: %d%%` NEM jelent:** a panel `scaletext` kijelzője (`0x00868e07` `× 100,0`) a **húzás-relatív** nagyítást mutatja (a húzás kezdetén beégetett `100`, `0x00868992`), és a `0x008685ca` `mov [edx+0x2c]` sem csomópontot ír — az a fogantyú-kezelő állapota. ⇒ **a `.cxf` `scale` nem százalék.** ⭐ **Egységes olvasat mind a hat témára:** a `scale` a csomópont **csempemérete lapegységben** — `picturepile`: `S(i)`; `regulargrid`: a cella szélessége; `contactsheet`: a lap-szintű csomópontmagasság; `multiexp`: 1,0 jelző. ⭐ **Új lelet a betöltési ágon:** a `FUN_00834520` (`0x00834520`) `picturepile`-ra szűkítve **megszorozza minden csomópont `+0x2c`-jét** (`0x00834683`–`0x0083469f`) — pontosan az az írási alak, amit a 19.2 hatóköre kimondottan nem fedett (`lea`-ba olvasztott cím). A mintáinkon nem látszik ⇒ a mi írónkra nincs következménye; a körbejárás-hűség kérdése külön jegy. ⛔ ~~**NYITVA marad** a `contactsheet` csempeméretének zárt képlete~~ ⚠️ **ELAVULT JELÖLÉS (2026-09-09, #2807/#2809):** a képlet a `kollazs-eletciklus.md` **18.2**-ben 2026-09-06 óta megvan, és a megnevezett keresési hely (a `CContactSheetTheme` saját csempeméret-írása) **nem létezik** — a `+0x3c` mind a kilenc előfordulása verem-lokális. Jegy: **#1412**, **#2593**, **#2583**.

⭐ **2026-09-06 (169. kör) — az Indexkép RÁCSA lezárva, és a `scale` írójának pásztázása egy KIMARADT ALAKKAL bővült (#1412, `kollazs-eletciklus.md` **19.**).** ⭐ **A `[this+0x18]` MEGVAN: a cellaél `k`, NEM a `scale`** — a 18.6 első feltevése megdőlt. A teljes rácsképlet `FUN_00887e50` (`0x00887e50`): `W' = CSONK(0,88·szélesség)`, `H' = CSONK(0,79·magasság)`, `k = CSONK(sqrt((W'·H')/n))` **egész** osztással a gyök előtt, `oszlop = W'/k`, `sor = H'/k`, majd `amíg (sor·oszlop < n): k--`; a kapu `szélesség/oszlop ≥ 8` ÉS `magasság/sor ≥ 8` (`0x008881ca`, `0x008881f1`), különben `-1`. ⚠️ A gyök eredménye is **csonkolva** (`or eax,0xc00` a `0x00888144`-en → `fistp` `0x00888156`). **Mérés 4/4:** k = 300 · 450 · 300 · 186, és a számolt rács (3×3 · 2×2 · 3×2 · 4×3) **egyezik a `.cxf`-ekből mért ráccsal**. ⭐ **ÚJ pásztázás — az egész `mov` alak**, amit az összes eddigi kör kihagyott (mind x87-et keresett): a teljes `.text`-en a SIB-alakú `mov [bázis+index+0x2c], …` **5 találat, a kollázs-sávban 0**; a disp8-as `mov [reg+0x2c], …` **608 / 96**, ebből **26** csomópont-alakú (ugyanaz a bázis ír `+0x20`-ba ÉS `+0x24`-be), ebből **5** áll 0x38-as lépésköz közelében — mind az öt elolvasva: **konstruktor/nullázás** (pl. `0x00829d60`, vtábla `0xcbf6a0`). ⇒ **a `scale` értékét EGYETLEN közvetlen tárolás sem írja**; a négy alak (x87 mutatós, x87 SIB, SSE/disp32, egész `mov`) együtt lefedi a közvetlen írást. ⚠️ **Hatókör kimondva:** a disp32-alakú `mov` és a `lea reg,[node+0x2c]`-be olvasztott cím NEM fedett. ⭐ **A SZERKEZETI lelet, ami megmondja, hol keressük:** az elrendezés **ideiglenes** vektorba megy (`FUN_00887ad0` lokálisai, a végén `FUN_0062d010` elpusztítja), és képernyő-elemeket hoz létre (`collagepanel/cnode_`) — a mentett `.cxf` geometriája mégis 31/31-ben egyezik ⇒ **van egy visszamásolás**, és ott kapja a `scale` az értékét; a csomópont értékadó operátora (`FUN_008341b0`) a `+0x2c`-t másolja (11. index a 14-ből). **A következő lépés: a `collagepanel/cnode_` elemek visszaolvasása — panel-kód, nem téma-kód.** Jegy: **#1412**, **#2583**.

⭐ **2026-09-06 (168. kör) — az Indexkép ELRENDEZÉSE TELJESEN KIMÉRVE, és a `scale` SZEREPE megvan (#1412, `kollazs-eletciklus.md` **18.**).** ⭐ **Két elrendezési képlet, 31/31 · 31/31 csomóponton PONTOS** (AI6 · AI27 · AI28 · AI29): `x = oszlop·cellaSzél + (cellaSzél − w)//2 + balMargó` és `y = sor·cellaMag + (cellaMag − **scale**)//2 + felsőMargó`. ⭐ **Az elrendező `FUN_00888210` (`0x00888210`)**, és mind az öt konstansa a binárisból kiolvasva: `0x00cf46d0` = **0,06** (oldalmargó), `0x00cf3fd0` = **0,15** (felsőmargó), `0x00cf4df0` = **0,08** (rés), `0x00d3a140` = **0,88** (cellaszélesség), `0x00d3a144` = **0,79** (cellamagasság). ⚠️ **CSONKOLÁS, nem kerekítés:** a kód minden `fistp` elé `or eax, 0xc00`-val nulla felé csonkoló FPU-módot állít (`0x00888258`, `0x008882a7`, `0x008882e6`, `0x00888323`) — kerekítéssel a cellaszélesség 451 volna, a minták **450**-et adnak. ⭐ **A `scale` SZEREPE (a #1412 eredeti rejtélye):** a lap-szintű csomópontmagasság a függőleges középre igazításhoz — ezért lap-szintű állandó. Két független megerősítés: a soron belül az `y` **azonos**, pedig a `h`-k különböznek (AI27: 446,08 és 449,75), és `max(w,h) ≤ scale` mind a 31 csomópontra. ⭐ **ÖNHELYESBÍTÉS a 17.1-hez:** a `scale` öt témában egyszerűen **max(w, h)** (`picturepile` 57/57, `regulargrid` 9/9) — a 17.1 arány-oszlopának 1,25 / 1,20020 / 1,78431 értékei **1/képarány**, nem témakonstansok. ⭐ **Lapegység:** 1024 × CSONK(1024·H/W), és `h = w / képarány` (eltérés < 0,001 egység, 19/19 nem-polaroid csomóponton). ⛔ ~~**NYITVA marad:** ki írja a `+0x2c`-t a layout UTÁN (a layout `0x3f800000` = 1,0-t ír), és mi a `scale` értékének képlete — a `scale`/cellaMag arány a két álló lapon 0,872 és 0,876, a két fekvőn 0,845 és 0,849, de **négy pontra konstanst illeszteni tilos**. A következő lépés a `[param_1+0x18]` harmadik rácsparaméter azonosítása.~~ ⚠️ **ELAVULT JELÖLÉS (2026-09-11):** mindkét gazdajegy LEZÁRULT, és a két kérdés külön sorsra jutott. A `scale` képlete **megvan**: a 170. kör (lásd feljebb) a `contactsheet` csempeméretét a `kollazs-eletciklus.md` **18.2**-re, a `picturepile` létráját pedig a `FUN_0082c9a0` kiolvasott képletére vezette. A `+0x2c` ÍRÓJÁNAK gépi keresése **kimerült**: a #2805 három címzési alakra (közvetlen, mutatós, indexelt) erősítette meg a negatív eredményt. A gyakorlati következtetés nem is további keresés, hanem hogy a mi kódunk a tárolt értéket **vigye át**, ne számolja újra — ez a **#2923** jegy. Jegy: **#2923** (a #1412 és a #2583 lezárva).

⭐ **2026-09-06 (20. kör) — a `.cxf` `scale` GÉPI keresése KIMERÜLT, és egy RÉS is kiderült a korábbi szűrőben (#1412, `kollazs-eletciklus.md` 17.15).** A jegy maga nevezte meg ezt az utat („marad a mutatós írási utak egyenkénti végigolvasása, tíz hely, ami sokkal drágább") — végigjárva. ⭐ **A kilenc sávon kívüli jelölt mind ELOLVASVA**, utasításonként: `0x0050bd70` és `0x0050be50` konstans **0,333**-at (`0xcf4030`) ír a `+0x1c`…`+0x24`-be és **0**-t a `+0x28`…`+0x30`-ba; `0x0050cdb0` **másoló** (`[esi+X] → [eax+X]`); `0x0050d560` a **szerkesztő csúszkája** (sztringje `editslider1/editslider`, egész osztása **255,0**-val); `0x005c2350` egységmátrix-init (`fld1` a `+0x14`-be); `0x007e68f0`/`0x007e6930`/`0x007e69b0` nulla-init; `0x009d7a60` konstans **−1,0** (`0xcf3ed0`). ⇒ **egyik sem SZÁMOL** — a 17.10 „kizárás hívási úton" megszorítása ezzel **tartalmi kizárássá** erősödött, a három gyengébben kizárt tétellel együtt. ⛔ **A RÉS, amit magamtól találtam:** a 17.10 szűrője **megkövetelte a `+0x28` írását is** (a `theta`+`scale` pár), tehát egy **csak `scale`-t** író függvény **kiesett volna a mintából**. Ez eddig kimondatlan volt. ⭐ **Bezárva:** új, `+0x28`-tól független pásztázás a `.text` teljes szakaszán (fájloffset `4096`, `8646656` bájt) a `fst`/`fstp dword ptr [reg + 0x2c]` **mutatós** alakra (`mod=01`, SIB és `ebp`-lokálisok kizárva) — **63 írási hely, 50 különböző függvényben**. A kollázs-sávban lévő, addig be nem sorolt ötöt elolvastam: `0x00829770` nulla-init; `0x00860f60` (sztringjei `Picasa`, `Arial`) és `0x00861190` (`Preferences`/**`PrinterQuality`**) **nyomtatás**; `0x0088e7e0` és `0x008910b0` konstans −1,0. ⇒ **a `+0x28` követelménye nem rejtett el semmit.** ⚠️ **A hatókör kimondva:** a **SIB**-alakú írások (`[reg + reg*8 + 0x2c]`) ebből a mintából kimaradnak — épp így ír a két téma-elrendező (`0x00885060` `regulargrid`, `0x00888210` `contactsheet`), amelyeket a 17.13 már kimért: **mindkettő `fld1`, állandó 1,0**. A két minta uniója fedi a csomópont-írás mindkét címzési alakját. ⛔ **A KÖVETKEZTETÉS MEGDŐLT, ugyanaznap** — ld. a következő bejegyzést. A szakasz akkor azt írta, hogy „a `Picasa3.exe`-ben nincs olyan kód, amely a `scale`-t kiszámolná”; **ténylegesen csak az dőlt el**, hogy a közvetlen `fst`/`fstp dword ptr [reg+0x2c]` alakok (mutatós ÉS SIB) egyike sem ír számított értéket. Jegy: **#1412**.

⭐ **2026-09-06 (19. kör) — az `outputlayout/export7_icon` RAJZA kimérve: tömör, felfelé mutató háromszög (#2537, `picasa-keptalca.md` 21.3).** A képtálca „További lehetőségek…" gombjának ikonja a #2493 óta hiányzott, mert a rajzot nem mértük ki — a jegy kimondottan tiltotta a találgatást. ⭐ **Kicsomagolva a `respack.yt`-ből** (`tools/picasa/respack.py png … export7`, réteg-eltolás **2 173 134**, 83 bájt): a kép **13 × 7**, és **egyetlen tömör alakzat** — soronként **1, 3, 5, 7, 9, 11, 13** képpont, a csúcs a tetején (x = 6), az alap a teljes szélességben az alján ⇒ **tömör ▲**. **49 látható képpont**, mind **`#69729B`** (RGB 105, 114, 155), **élsimítás nélkül**. ⛳ **A tájolás nem feltevés:** a kicsomagoló sorrendjét két, egyértelmű tájolású testvér-ikonnal ellenőriztem — a `folderbutton_icon` (17 × 13) füle a bal **FELSŐ** sarokban, a `pbutton_icon` (15 × 12) papírja **felül**, a kiadó tálcája alul ⇒ a sorok **nincsenek tükrözve**. A fölfelé mutató csúcs a funkcióval is egybevág: a képtálca az ablak **alján** ül, a túlcsordulás-lista **fölfelé** nyílik. ⭐ **Melléklelet — a sávnak EGYETLEN tintaszíne van:** mind a tizenegy `outputlayout` ikon fő színe **`#69729B`**; a többi ikon ezt **élsimítással** hígítja (3–15 szín), az `export7_icon` az **egyetlen**, amiben csak **1** szín van. Kivétel a `sharewith_icon` (40 × 21, **279** szín) — az nem vonalas piktogram. ⛔ **Nálunk (MÉRVE):** a `TrayBar.qml` `trayMoreButton`-ja (`:1672`) ma **ikon nélkül**, csak a felirattal áll — a #2493 óta szándékosan, mert a hiányzó `icons/export.svg`-re mutató hivatkozás minden induláskor `QQuickImage: Cannot open` hibát írt. ⇒ a rajz most **mérés alapján** elkészíthető (`icons/tray-more.svg`, 13 × 7 arány, `#69729B`), a találgatás tilalma sértése nélkül. Jegy: **#2537**.

⭐ **2026-09-06 (18. kör) — a UI-lefedettségi mérés FELTÁRATLAN oszlopa NULLA (#2529 LEZÁRVA), és a `movieeditpanel` hármasa kimérve.** ⭐ **A `movieeditpanel/reset_trim` felirata FÉLREVEZET:** „Reset Start and End" / „Kezdés és befejezés alaphelyzetbe állítása", a művelet viszont a **`CThumbUI::UndomovieEdits`** (`0x005ef3e0`), ami **„Remove all movie edits?"** megerősítést kér (`IDS_CONFIRMREVERT`, igen-gomb „Remove Edits") és **az ÖSSZES film-szerkesztést** eltávolítja. Bizalmi fok: **erős** (hívás + sztringkészlet); hogy pontosan mely `filters=` tokeneket törli, **NINCS MÉRVE**. ⭐ **A `movieeditpanel/capture_frame` a pillanatnyi lejátszási pozícióból dolgozik:** `[panel+0xe64]` → `fdiv 0xcf3e88` (**= 10 000 000,0**, ugyanaz a 100 ns-os egység, mint a `rewind`-nél), a képkocka a **„Rögzített videoklipek"** mappába megy. ⭐ **A `movieeditpanel/export_movie`** a `0x0053a460`-t hívja: **`CFileSaveThread`** háttérszál, célmappa **„Exportált videoklipek"** (`CThumbUI::MovieClipFolder`), és **platform-kapu**: `LinuxNomovie` — „A program ezt a funkciót Linux rendszeren nem támogatja". ⭐ **A parancs-elosztó `0x005952d0` mind a kilenc ága** kiírva (`play`, `pause`, `moviemode1`, `setin`, `setout`, `reset_trim`, `capture_frame`, `export_movie`, `export_youtube`); a `setin`/`setout` írja a **`moviestart`**/**`movieend`** tokent, az állapotfrissítő (`0x00596980`) pedig a két vágás-fogantyút, a csúszkát **és a `reset_trim`/`export_movie` engedélyezettségét** állítja. ⭐ **Az `instructionpanel/learn_more` célja kimérve:** a `0x0074c760` a `http://picasa.google.com/support/bin/answer.py?hl=%s&answer=156272` címet nyitja (`%s` = nyelvkód) ⇒ megszűnt súgószolgáltatás, hatókörön kívül. ⭐ **A `gedialog/done`** kezelője a `0x00854080` (94 b): időzítő-kapu (`GetTickCount` vs `[this+0x264]`), majd `0x00853f90`, végül **magát az elemet aktiválja** (`0x009cd8a0`) ⇒ a párbeszéd lezárása, adatot nem ír; a hatókörön kívüli besorolás így már **mérésen** nyugszik. ⛔ **A maradék hét elem SOHA NEM VOLT feltáratlan** — negyedszer ugyanaz a hibafajta: a `jobb-fiok-meretek.md` (`rightdrawerpanel/size_toggle`, `title_text`, `geopanel/search_label`, `peoplepanel/status_label`), a `picasa-importalas.md` (`acquirepanel/nextbutton`, `previousbutton`) és a `picasa-menu-parancsok-viselkedes.md` 63.3 (`gedialog/done`) mind **levélnevet** használt, a `picasa-importalas.md`-n ráadásul **egyetlen horgony sem** volt az egész lapon. Teljes nevekre és `.tre`-horgonyokra javítva. ⛔ **Új, megnevezett nyitott kérdés:** a `rightdrawerpanel/size_toggle` kezelője megvan (`0x00632060`, a `close`-zal közös, **puszta** névre hasonlít), de **NINCS MÉRVE**, melyik két képpont-szélesség közt vált; a megszerzés útja a lapon ki van mondva. ⇒ **feltáratlan 11 → 0**, lekutatva 281 → **294**, bizonytalan 49 → **47**, lefedettség **42,1%**. ⛳ **Amit ez NEM jelent:** a `lekutatva` annyit tesz, hogy egy spec **horgonyzott szakaszban, teljes néven** megnevezi az elemet — nem azt, hogy a viselkedése végig ki van mérve. A 47 `bizonytalan` elem is nyitott munka. Jegyek: **#2529** (LEZÁRVA), **#2536** (85 horgonytalan szakasz, 128 elem — ez a hibafajta gyökere), **#1838** (a videó-vágás megépítése).

⭐ **2026-09-06 (17. kör) — az `edittextpanel` öt felirata SOHA NEM VOLT feltáratlan: a geometriájuk évek óta le van írva, csak HORGONY NÉLKÜL (#2529, `szerkeszto-panel-meretek.md` 6.4).** ⛔ **A kör premisszája megdőlt.** A #2529 az `edittextpanel` öt feliratát (`align_label`, `edittext_label`, `size_label`, `style_label`, `transparency_label`) kutatói feladatként adta át. A mérés szerint a `szerkeszto-panel-meretek.md` **6.4** szakasza — a panel **21 soros, teljes geometriai táblája** — mindegyiket tartalmazza méretben és koordinátában; a mérő azért nem látta, mert a tábla **levélneveket** használt ÉS a szakaszban **egyetlen horgony sem** volt. ⭐ **A tábla teljes nevekre és horgonyokra javítva** (`edittextpanel.tre:7`…`:158`), és **kiegészítve azzal, ami tényleg hiányzott: a feliratok szövegével, betűmakrójával és igazításával.** Mind a hat: `edittext_label` „Edit Text"/**„Szöveg szerkesztése"** `m_displayfont18_Reg` (`:10`); `font_label` „Font:"/**„Betűtípus:"** (`:14`); `size_label` „Size:"/**„Méret:"** (`:24`); `style_label` „Style:"/**„Stílus:"** (`:37`); `align_label` „Alignment:"/**„Igazítás:"** (`:70`) — ez a négy `m_displayfont12` + **`Property textalign right`**, tehát a vezérlőjétől **balra**, jobbra igazítva; és `transparency_label` „Transparency"/**„Átlátszóság"** (`:118`), ami **KIVÉTEL: `Property textalign center`**. ⛳ **A kivétel kétszeresen igazolt:** a `.tre` középre igazít, és a geometria is ezt mondja — a felirat (127 × 15, x 79..206, y 202) **pontosan a csúszka fölött** ül (127 × 27, x 79..206, y 217), **azonos szélességgel**. A hivatalos magyar feliratok forrása a Picasa saját erőforrása (`panel-feliratok-hu.tsv:247–258`) — nem fordítandó újra. ⛔ **Nálunk (MÉRVE, `EditorTextPanel.qml`):** a panel megvan, de **szakaszcímeket** használ a vezérlők FÖLÖTT, nem címkeoszlopot balra: „Text" (`:31`) az „Edit Text" helyett, „Font" (`:79`) a „Betűtípus:" helyett, „Opacity" (`:234`) az „Átlátszóság" helyett, a „Méret:", „Stílus:" és „Igazítás:" pedig **egyáltalán nincs meg** (csak buboréksúgó). ⇒ elrendezési eltérés, nem szóhasználat. Jegy: **#2535**. ⭐ **A HIBAFAJTA MÉRETE — új jegy, #2536:** a mérő #38 óta figyelmeztet rá, de jegy nem volt hozzá: **85 spec-szakasz** nevez meg felületi elemet teljes néven, **horgony nélkül**, és emiatt **128 elem** látszik feltáratlannak. Ez a harmadik előfordulás (#2182: 12 elem, #2504: `thumbui/single_action_message`, most a 6.4). A jegy a **teljes**, 85 soros listát tartalmazza, azzal a figyelmeztetéssel, hogy a találat **jelölt, nem bizonyíték**: van köztük puszta felsorolás, oda horgonyt tenni hamis `lekutatva`-t adna. ⇒ **feltáratlan 16 → 11**, lekutatva 274 → **281**, bizonytalan 51 → **49**, lefedettség **42,1%** (változatlan — a `hianyzik`→`lekutatva` átsorolás a nevezőt nem mozdítja). Az `edittextpanel` feltáratlan oszlopa **0**. A maradék 11: `movieeditpanel` 3, `rightdrawerpanel` 2, `acquirepanel` 2, `peoplepanel`/`instructionpanel`/`geopanel`/`gedialog` 1-1. Jegyek: **#2529** (nyitva marad), **#2535** (a hat felirat megépítése), **#2536** (a 85 horgonytalan szakasz).

⭐ **2026-09-06 (16. kör) — a Feltöltéskezelő KIMÉRVE, és a megszűnt online elemek `nem-cel`-re: feltáratlan 31 → 16 (#2529, ÚJ LAP: `picasa-feltolteskezelo.md`).** ⭐ **A `CUploadManagerDialog` hat vezérlője, működés szerint** (elosztó `0x007d2400`, 1509 b; a motor `CUploadManager`, RTTI `0x00ca7380`): `pause` → `[motor+0x102] := 1` + `[[motor+0x50]+0x20]` (`0x007d2820`); `resume` → `0x00685c20` (`0x007d286a`); `cleanup` → végigmegy a `[motor+0x274]` sorlistán, és a **7-es állapotú** (kész) elemeket veszi ki (`cmp dword ptr [edi+ecx+0x10], 7`, `0x007d28c6`); `hide` → `[[this+0x258]+0xc]` (`0x007d294e`); `minibutton` → `0x007d2d80`, ami **`"%s:%d"`** alakú nevet gyárt (`0x00cb8e2c`) és azzal keres elemet a felületi fában (`0x00c07db2`); `itemlist` → a sor maga, a soronkénti műveletek generált nevekkel a `0x007d2e60`-ban (`upmgrlist-view`/`-cancel`/`-retry`/`-clear`/`-showerr`). ⭐ **AZ EGYETLEN TARTÓS ÁLLAPOT a `throttlechk`:** `0x00917ce0` → `HKEY_CURRENT_USER\SOFTWARE\Google\Picasa\Picasa2\Preferences\AutoBandwidthThrottle` (kulcsnév `0x00cd1a88`), **alapérték 0** (`0x00917d00` `mov dword ptr [esp+0x14], 0`), azonnali írás (`0x00401900`) és visszaolvasás (`0x004019b0`) a globális `[0x00d676dc + 0xd4]` bájtba. A másik öt vezérlő csak futásidejű állapotot állít. ⭐ **Belépési pontok:** Eszközök ▸ „&Feltöltéskezelő…" (`eMenuTools::ID_TOOLS_UPLOADMGR`, `stringres-en-hu.tsv:2731`, menüépítő `0x00559150`) és a tevékenység-gomb (`activity/activitybutton`, `0x007d3f90`). A HU feliratkészlet **hiánytalan**, 30 kulcs (`uploadmgr::*`, `:3304`–`:3333`). ⭐ **14 elem `nem-cel`-re** — mind a **Picasa Web Albums** feltöltéshez tartozik: `uploadmgr` 6 (`pause`, `resume`, `cleanup`, `itemlist`, `minibutton`, `throttlechk`), `headerpanel` 4 (`sync_label`, `sync_options`, `view_online`, `websync1`), `acquirepanel` 4 (`add_groups_button`, `selected_groups_label`, `share_with_label`, `upload_label` — a megosztás-blokk a `0x00518840`/`0x005154f0`-ben, az `add_groups_button` az importáló elosztójában, `0x0051f070`). ⛳ **A besorolás nem a kör döntése:** a tulajdonosi „Megszűnt szolgáltatások — ezek nem is fognak elkészülni" lista (`src/picasapy/help/features/meg-nem-erheto-el.md`) és a `PicasaMenuBar.qml:1596` `retired: true` (#638). ⛔ **A `movieeditpanel/export_youtube` NEM `nem-cel`:** a felirata alapján tudjuk, mit csinál („Upload to YouTube" / „Feltöltés a YouTube webhelyre"), tehát **`lekutatva`** — de hogy megépítjük-e, az **termékdöntés**, nem kutatás; a #2529-en marad. ⛳ **Módszertani mellékelet kimondva:** ez az elem azért került át a feltáratlan oszlopból, mert az új lap teljes néven említi horgonyzott szakaszban — pontosan az a mechanizmus, amit a #2504 figyelmeztetője mér; itt szándékos, de a mérő nem tudja megkülönböztetni a szándékos említést a véletlentől. ⇒ **feltáratlan 31 → 16**, nem cél 77 → 91, lefedettség **41,2% → 42,1%**. A maradék 16: `edittextpanel` 5, `movieeditpanel` 3, `rightdrawerpanel` 2, `acquirepanel` 2, `peoplepanel`/`instructionpanel`/`geopanel`/`gedialog` 1-1. Jegy: **#2529** (nyitva marad).

⭐ **2026-09-06 (15. kör) — a UI-lefedettségi mérés újrafuttatva, és a valódi ok NEM az osztályozó volt, hanem HAT elavult KÉZI FELÜLBÍRÁLÁS (#2504, `ui-lefedettseg.md` + `ui-lefedettseg-elemek.csv`).** ⛔ **HELYESBÍTÉS a #2504 saját premisszájára és a 6. kör index-sorára.** A jegy azt írta, hogy két elem azért áll „feltáratlan"-ként, mert az osztályozó a **levélnevet** nem találja meg horgonyzott szakaszban. A friss futás megmutatta, hogy ez a `thumbui/hviewtoggle`-re **nem igaz**: az elem TELJES néven, horgonyzott szakaszban le van írva (`picasa-fo-ablak-elrendezes.md` 4., `thumbui.tre:406/412/415`) — az osztályozó megtalálta volna, csak egy **kézi `hianyzik` felülbírálás** írta felül. ⛔ **A 2026-09-05-i index-sor is hibás volt:** „egyetlen `hiányzik`" helyett a `ui-lefedettseg-elemek.csv`-ben **HAT** ilyen tétel állt — `thumbui/hviewtoggle`, `editpanel/movietab`, `editpanel/movietabpanel`, `editpanel/modaldialogblur`, `editpanel/picnikapply`, `editpanel/preview2`. ⭐ **Mind a hatnak volt bizonyítéka** a saját `bizonyitek` oszlopában (`editpanel.tre:476` / `:484` / `:1362` / `:1430` / `:1066`, illetve `0x00593dcf`, `0x00593880`), **és** teljes néven, horgonyzott szakaszban leírt spec-szakasza (`ui-audit-editor.md` 2317., 2582–2590., 2643–2645. sor) ⇒ **mind a hat `lekutatva`, nem feltáratlan** — a fogalmi hiba az volt, hogy a `hianyzik` állapotot „nálunk nincs meg" értelemben használtuk, holott a mérésben „nem tudjuk, mit csinál" a jelentése. Átsorolva, indoklással a `megjegyzes` oszlopban. ⭐ **A mérés eredménye a javítás után:** feltáratlan **37 → 31**, lekutatva **267 → 273**, lefedettség **41,2%** (alsó becslés; 51 `bizonytalan` elemmel 49,0% lenne), és **az öt vezető panel feltáratlan oszlopa 0 / 0 / 0 / 0 / 0** — a #2504 elfogadási feltétele teljesült. ⭐ **ÚJ MÉRŐ a `ui_lefedettseg.py`-ban (#2504 opcionális pontja):** figyelmeztet, ha egy elem **levélneve** megvan horgonyzott szakaszban, a **teljes neve** viszont nem — pontosan ez a néma hibafajta tette évekig „feltáratlan"-ná a `getmore-klipgyujto-mod.md`-ben leírt `thumbui/single_action_message`-t. A zajszűrő MÉRT: a 2020 elem 1580 különböző levélnevet használ, ebből **1312 pontosan egy elemhez tartozik** — csak ezekre szólunk —, és közülük **27 legfeljebb 4**, **123 legfeljebb 6** karakteres hétköznapi szó (`crop`, `date`, `name`, `zoom`), ezért a nyolc karakteres alsó határ. ⛳ **A minta bal oldalát is határolni kellett:** az első futás `collagepanel/action_group`-ot jelzett a `thumbui/single_action_group`-ról szóló szakaszra, mert a meglévő `_szohatarral()` csak a záró oldalt őrzi (a teljes neveknél elég, a levélneveknél nem) ⇒ új `_szohatarral_ketoldalt()`. ⇒ **A 6. kör „a UI-lefedettségi axisból kutatói kör már nem tud témát venni" állítása MEGDŐLT:** 31 feltáratlan elem maradt, más paneleken — `uploadmgr` 6, `acquirepanel` 6, `edittextpanel` 5, `movieeditpanel` 4, `headerpanel` 4, `rightdrawerpanel` 2, `peoplepanel`/`instructionpanel`/`geopanel`/`gedialog` 1-1. Jegyek: **#2504** (LEZÁRVA).

⭐ **2026-09-06 (14. kör) — az automatikus kiegészítés FORRÁSA megvan, és a kapcsolónak NINCS OLVASÓJA (#2524 LEZÁRVA, `picasa-menu-parancsok-viselkedes.md` 24., `picasa-ini-format.md`).** ⭐ **A mező gazdája azonosítva:** a `ComposeDialog+0x26c` egy **`CEmailAutoComplete`** (RTTI-vtábla `0x00cc2f90`, építő `0x00858480`), amely a `+0x15c`-en **`INotifyContactsDownload`**-ot is örököl (vtábla `0x00cc3070`, egyetlen metódus: `0x00857d00`, a thunk `0x00857d0d` `lea esi,[ebx-0x15c]`). ⇒ **`ComposeDialog+0x3e0` = `CEmailAutoComplete+0x174`**, és a mezőnek **saját alapértéke is 1** (`0x0085850e`). ⛔ **NEGATÍV, KIMERÍTŐ — az „Automatikus kitöltés" kapcsolónak HÁROM írója és NULLA olvasója van.** Három független pásztázás a `.text` teljes szakaszán (fájloffset `4096`, `8646656` bájt), a közvetlen operandusú alakokat is beleértve: `disp32 = 0x174` (24 találat), `disp32 = 0x3e0` (45), `disp32 = 0x128` (a második bázisra vetített alak, 6) — egyik sem olvasó; ezen felül a **teljes osztálytörzs** (`0x00857d00`–`0x0085a2d0`, ~8,7 KB) utasításonként átnézve csak a `+0x4`, `+0x8`, `+0xc`, `+0xd`, `+0x68` eltolásokon fér bájthoz. ⇒ **a Picasa 3.9.141.259-ben a kapcsoló eltárolja a beállítást és átállítja a pipát, de a javaslómotor soha nem kérdezi meg** — a #1526 megvalósítása tehát NE kapuzzon vele semmit. ⭐ **A javaslatlista:** `+0x160` adatmutató, `+0x164` **elemszám × 2** (`0x00858cc0` → `shr edx,1`), **24 bájtos** lépésköz (`0x00858d06` `lea eax,[eax+eax*2]` → `0x00858d0d` `lea ecx,[ecx+eax*8]`); a letöltött névjegyek külön tömbben, `+0x168`/`+0x16c`. ⭐ **A címek KÉT forrása, mindkettő mérve:** (1) **online** a GData névjegy-adatfolyam — `%sm8/feeds/contacts/%s/full?max-results=10000` (`0x0078fe50`), **legfeljebb 10 000** névjegy menetenként, a felület `/c/ui/ContactManager` (`0x0085a1a0`); (2) **helyben** a **`.picasa.ini` `[Contacts2]`** szakasza (író `0x00586e20`, olvasó `0x00587110`, a hívó `0x0045a9d0` egyetlen sztringje `.picasa.ini`). ⭐ **A `[Contacts2]` érték PONTOS formátuma:** kulcs a névjegy 64 bites azonosítója `%I64x` (`0x00c82fcc`), érték `%s;%s;%s` (`0x00c91104`, elválasztó `;` = `0x00c81320`) = **`full_name;email;gaia_id`** — a mezőnevek nem a naplószövegből következtetve, hanem az író `push edi; push esi; push eax` sorrendjéből (`0x0058705d`–`0x00587060`) a naplóhívás regisztereivel egyeztetve (`0x00c910a8`, `0x00586f9d`–`0x00586fb1`). **Pontosan három token kötelező** (`0x00587203` `and eax,0xfffffffe` → `0x00587206` `cmp eax,6` a token×2 alakon), különben a bejegyzés **eldobódik**. ⇒ a `Roy Avery;;` alak **kitöltetlen `email` és `gaia_id`**, nem töltelék. ⛔ **Nálunk (MÉRVE):** az `ini/contacts.py` írója helyes (`Név;;`, három token), az olvasója viszont **bármennyi tokent elfogad** (`contacts.py:28`), és a két mezőt névtelen `extra`-ként tartja (`contacts.py:20`). Jegyek: **#2524** (LEZÁRVA), **#2526** (a mezőnevek és a hármas szabály nálunk), komment **#1526**.

⭐ **2026-09-06 (13. kör) — az `Address::ID_AUTOCOMPLETE` VÉGIG kimérve: pipás tétel, registry-kulcs, alapérték, és mind a négy tiltási feltétel (#1526, `picasa-menu-parancsok-viselkedes.md` 23.).** A 19. adag csak megnevezte a tételt (»kikapcsolható automatikus kitöltés«); hogy **mit** kapcsol, **hova** menti és **mi az alapértéke**, nem volt mérve. ⭐ **A kapcsoló:** `0x00851b50` — `sete` invertál, a `[this+0x5b8]` bájtba ír (`0x00851b5e`), majd **azonnal** a registrybe: `HKEY_CURRENT_USER\SOFTWARE\Google\Picasa\Picasa2\Preferences\EmailAutocomplete` (kulcsnév `0x00cc2640`, szekció `0x00c7eafc`, író `0x00401900`). ⇒ **a kulcs neve elárulja, hogy az `Address` névtér az e-mail-címmező** — összeér a `picasa-email-kuldes.md` beállítás-készletével. ⭐ **Alapérték = 1 (BE)**, két független forrásból: az építő `mov byte ptr [edi+0x5b8], 1` (`0x0084fe43`), és az induláskori registry-olvasás alapértéke is `1` (`mov edi, 1` a `0x0085021a`-n, átadás `0x008503e6`, olvasás `0x004019b0`). ⭐ **Pipás menütétel, nem gomb:** `CheckMenuItem` (`0x00c40810`) `MF_CHECKED` (8) / `MF_UNCHECKED` (0) a `[this+0x5b8]` szerint (`0x008519b3`–`0x008519c8`), és **soha nem tiltott**. ⭐ **Mind a hét parancsazonosító + a végrehajtott üzenet:** `ID_UNDO` `0x9d7f`→`WM_UNDO`, `ID_CUT` `0x9d39`→`WM_CUT`, `ID_COPY` `0x9d3b`→`WM_COPY`, `ID_PASTE` `0x9d3c`→`WM_PASTE`, `ID_DELETE` `0x9c56`→**`WM_CLEAR`**, `ID_SELECTALL` `0x9d7e`→`EM_SETSEL(0,−1)`, `ID_AUTOCOMPLETE` `0x9d82`→`call 0x00851b50`. A menü `TrackPopupMenu`-val jön fel, `TPM_RETURNCMD` (`0x10a`, `0x00851a45`). ⭐ **A NÉGY tiltási feltétel kimérve** (`EnableMenuItem`, `MF_GRAYED`): Visszavonás ← `EM_CANUNDO` = 0 (`0x00851902`); **Kivágás + Másolás + Törlés EGYÜTT** ← `EM_GETSEL` kezdet = vég (`0x0085193c`); Beillesztés ← `OpenClipboard`/`GetClipboardData(CF_TEXT)`/`CloseClipboard` NULL-t ad (`0x0085196e`–`0x0085197e`); Az összes kijelölése ← `WM_GETTEXTLENGTH` = 0 (`0x0085199f`). ⛔ **Hatókör:** ezek az `Address::` névtér azonosítói; a `picasa-gyorsbillentyuk.md` `Ctrl+X`/`Ctrl+C`/`Ctrl+V` sorai az `eMenuEdit` névtérre vonatkoznak, azt ez a mérés **nem** dönti el — ott marad a »nincs mérve«. ⛔ **Nálunk (MÉRVE):** a hét tétel megvan azonos sorrendben (`app/qml/PicasaPy/TextFieldContextMenu.qml`), és **mind a hat tiltható tételnél egyezik a feltétel** — ezt eddig senki nem mérte. Egyedül az »Automatikus kitöltés« **halott helyfoglaló**: nincs `checkable`, nincs `onTriggered`, nincs tárolt állapot. ⛔ **Kimerítő pásztázás, névvel:** a `.text` teljes szakaszán a `+0x5b8`-ra **6** bájt-hozzáférés (3 író, 3 olvasó), a `+0x3e0`-ra **2 — mindkettő ÍRÓ, olvasó nincs**. Ez nem halott mező (a fordító összevonhatja a bázis+tag konstanst, ld. a `facerect`-kört), de a **tényleges kiegészítő logika NINCS MEG**. Jegyek: **#1526** (komment), **#2524** (a `+0x3e0` olvasója és a címforrás).

⭐ **2026-09-06 (12. kör) — a másolás LEVESZI az írásvédettséget, és egy ELAVULT „nincs mérve" jelölés eltávolítva (#440, `biztonsagi-mentes.md` 11.1/d).** ⛔ **A kör ELAVULT jelölésből indult:** a lap „Bizonyítottsági fok" mondata még azt írta, hogy „a `files.txt` **tartalma** nincs mérve" — holott ugyanennek a lapnak a **11.1** (2026-09-03) és **13.3** (2026-09-04) szakasza már megmérte, és a **#2090 LEZÁRULT**. A jelölés eltávolítva. ⛳ **A zárt jegy egy TÉVES állítástól mentett meg:** a kör a `0x00677a70` másolóból azt olvasta ki, hogy a `files.txt`-et `OPEN_ALWAYS`-szel nyitja (tehát nem csonkol) — a #2090 viszont kimondja, hogy az **író** a `0x008447b0`, `"w"` módban. A két megállapítás más fájlra vonatkozik; a jegy nélkül ez ellentmondásként ment volna ki. ⭐ **ÚJ, mért lelet:** a **szokásos** másolási ág (a 11.1/a kapu `je 0x00677f63` ága) a cél megnyitása előtt **leveszi az írásvédettséget**: `GetFileAttributesEx` (`0x00677f6d`, `0xd69518`) → `test byte [esp+0xd4], 1` (`0x00678334`) → `GetFileAttributes` (`0x0067834e`, `0xd694bc`) → `and eax, 0xfffffffe` (`0x0067835d`) → `SetFileAttributes` (`0x00678362`, `0xd69514`) → `CreateFile(GENERIC_READ|WRITE, FILE_SHARE_READ|WRITE, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL)` (`0x0067839a`). ⇒ **írásvédett célfájlon a Picasa nem hibázik, hanem törli a jelzőt és felülír** — egy naiv másoló itt `PermissionError`-ral állna meg. ⛔ **FELTÉTELES:** hogy a `[esp+0x1c]` ezen a ponton pontosan melyik útvonalat tartja (a rekeszt a `0x00677c17` újraírja); a következő lépés a `0x00677c17` forrásának visszakövetése. Komment: **#440**.

⭐ **2026-09-06 (11. kör) — a `makemoviepanel` PARANCS-ELOSZTÓJA és a `rewind` MŰKÖDÉSE (#432, `picasa-create-features.md` 2.12).** ⛔ **MEGDŐLT egy korábbi lezárás:** a 2026-09-05-i bejegyzés szerint „**NINCS MÉRVE**, mit csinál a `rewind` gomb kattintásra (az olcsó lánc kimerült)". A lánc **nem** merült ki — a kör a **minősített** nevet (`makemoviepanel/rewind`) kereste, az elosztó viszont a **puszta** nevet hasonlítja (`rewind`, `0x00c9d3dc`), és annak egyetlen hivatkozója a **`FUN_0061df10`** (12 420 b), a panel **parancs-elosztója**. ⭐ **Mind a 34 parancs kiolvasva** az elosztó sorrendjében, címmel (`recompute`, `render`, `cancel`, `export_youtube`, `play`, `pause`, `rewind`, `add_audio`, `remove_audio`, `next`, `prev`, `insert_slide`, `remove_slide`, `viewedit`, `fullscreen`, `aspectratiochk`, `1to1`, `show_captions`, `crop_to_fit`, `remove_low_res_faces`, három rendezés-rádió, `tab1`–`tab3`, `addclips`, `addtomovie`, `deleteclips`, `bold`, `italic`, `outline`, `autoplay`) ⇒ a panel teljes vezérlő-készlete a **működés** oldaláról. ⭐ **A `rewind` utasításonként** (`0x0061e3b7`–`0x0061e45a`): (1) **megállítja a lejátszást** — a `video_control_bar2/moviecontrols/pause` elem **aktiválásával** (`FUN_009cd8a0` névre keres a felületi fában, majd a `[vtbl+0x78]`-at hívja); (2) kiszámolja a **kijelölt dia kezdő időpontját** (`FUN_0080ff20`); (3) `× 10 000 000` (`0x00cf3e88`) ⇒ **100 ns-os** egység, és a lejátszó `[vtbl+0x5c]`-jével **odatekeri**; (4) `[+0x388] := [+0x494]` — az aktuális dia a kijelölt lesz; (5) felület-frissítés (`0x00619ac0`, `0x0061ca30`). A `+0x498` jelző a menet idejére be van kapcsolva (a saját pozíció-változtatás ne hasson vissza a kijelölésre). ⛔ **HELYESBÍTÉS:** a `0xF4240` visszatérési kód **nem** az adott ág sajátja — a `.text`-ben **809** helyen szerepel, tehát általános „kezeltem" kód; a `picasa-imagedata-rekord.md` `facerect`-szakaszát ezért átvezettem. ⛔ **Nálunk (MÉRVE):** interaktív filmkészítő panel **nincs** (`movie/slideshow.py` egy export-függvény, `grep -rn "rewind" src/` → 0). Komment: **#432**, **#2503**.

⭐ **2026-09-06 (10. kör) — az `imagedata` rekord TELJES típus-táblája, és a `pmp-database.md` „regisztrálatlan slot" kérdése LEZÁRVA (#2520).** ⭐ **Mind a 44 oszlop TÍPUSA megvan**, nem következtetésből: a regisztráló hívás **célcíme** adja meg, és mind a **nyolc** konstruktor RTTI-vel azonosítva — `0x004941f0` `ytString` (**0x00**), `0x00494c50` `unsigned long` (**0x01**), `0x00495d30` `double` (**0x02**), `0x00496020` `signed char` (**0x03**), `0x00495360` `unsigned __int64` (**0x04**), `0x004961b0` `unsigned short` (**0x05**), `0x00493ce0` `char const*` (**0x06**), `0x00495ec0` `int` (**0x07**). ⇒ a #2105 típustáblájának és a #2106 előjelesség-javításának **teljes** megerősítése (eddig három típusra volt meg, most mind a nyolcra). ⭐ **LEZÁRVA a `pmp-database.md` nyitott tétele:** a lap eddig azt írta, hogy a `0xc0`-nál nagyobb lépésköznél „regisztrálatlan slot marad ki… Hogy azok mik, nincs mérve" — **téves**, mind a hat rés egy-egy (a `lat`/`long`-nál kettő) **regisztrált** oszlop: `name` `+0x1cc`, `size` `+0x2f0`, `crop64` `+0x5b0`, `text` `+0x7f8`, `tags` `+0x8b8`, `lat` `+0xb70`, `long` `+0xbd8`. A lépésköz ezzel egyenletes (`0x60`/`0x68`). ⭐ **Amit a típusok elárulnak:** a `rotate` és a `flipped` az adatbázisban is **sztring** (a `.picasa.ini`-beli `rotate(N)` alak), a `lat`/`long` **`double`** (fokban), az `edit_width`/`edit_height` **előjeles `int`**, a `tags` az **egyetlen `char const*`** oszlop, a `crop64` pedig **ugyanaz a u64**, mint a `facerect` — és ugyanazzal a `FUN_009b9150` csomagolóval íródik ⇒ **a vágási téglalap bitszerkezete azonos az arc-téglalapéval**. ⛔ **NEGATÍV: a `star` NEM regisztrált oszlop** — a 3.9 a csillagozást a **`starlist.txt`**-ből olvassa (`0x00c81ad4`; a perzisztáló `0x0041ba40` és `0x004a82d0`), tehát az `imagedata_star.pmp` **örökölt**. ⇒ a `pmpimport/importer.py` kétforrású megoldása igazolt. ⛔ **Nálunk (MÉRVE):** a 44-ből **kilencet** olvasunk (`importer.py:26`). Jegy: **#2521** (a PMP-importőr oszlop-típus ellenőrzése); PR **#2520**.

⭐ **2026-09-06 (9. kör) — az `1`-es jelző ÍRÓJA megvan a felhasználói kiváltó okkal, és a mezőszám MEGINT helyesbítve (#2515 LEZÁRVA).** ⛔ **HELYESBÍTÉS (másodszor):** az `imagedata` rekord **44** mezős — a lap eredetileg 38-at, az előző kör 37-et írt. A 37-es mérés **hét** mezőt hagyott ki, mert a minta csak a `lea eax` alakot ismerte; a hét: **`name`, `size`, `crop64`, `text`, `tags`, `lat`, `long`** — a nevüket a `string_xrefs` sem adta vissza, **közvetlen `.rdata`-olvasás** hozta elő (`0x00c7fa20`, `0x00c80a9c`, `0x00c80adc`, `0x00c80b0c`, `0x00c80b20`, `0x00c80b6c`, `0x00c80b70`). Ezek fele **`.picasa.ini`-kulcs is** (`crop64`, `text`, `tags`, `lat`, `long`). ⭐ **AZ `1` ÍRÓJA:** `FUN_00446960(gazda, sorlista, bájt, jelző)` — a bájtot **előjelesen 64 bitre kiterjesztve** teszi a `facerect` sorába (`0x004473f1` `movsx`, `0x00447402` `cdq`, `0x00447403` alsó / `0x0044740a` felső 32 bit), a tömböt szükség szerint újrafoglalva (`0x00447387`). ⭐ **A KIVÁLTÓ OK a felhasználóig kikövetve:** `FUN_007c4df0` (**`CFolderMgrDialog`**, a `"Are you sure you want to remove all faces and name tags from excluded folders?"` és a `CFolderMgrDialog::confirmfrexclude` sztringgel) → `FUN_005cef20` → `FUN_00491210`, ami `0x00491627`-nél **`1`**-gyel, `0x00491636`-nál **`0`**-val hívja a beállítót. ⇒ **a Mappakezelőben az arcfelismerésből kizárt mappák képei kapják az `1`-et**, és mivel az író (`FUN_00480040`) **csak nulla értékre ír**, az `1` **megvédi a képet az újra-detektálástól** — ez a jelző értelme. ⭐ **A `crop64` oszlopnak külön írója van:** `FUN_0047e930` ugyanazzal a téglalap → `FUN_009b9290` → `FUN_009b9150` lánccal ír a `+0x14d0`-ra (`0xf20 + 0x5b0`). ⛔ **NEM mértem:** melyik kép kerül a `FUN_00491210` „A", és melyik a „B" sorlistájába. Jegy: **#2515** (LEZÁRVA); a Mappakezelő lapja is átvezetve.

⭐ **2026-09-06 (8. kör) — a `facerect` HÁROM ÁGA és az ÍRÓJA megvan (#2515, `picasa-imagedata-rekord.md`).** ⛔ **HELYESBÍTÉS az előző körre:** a „bájtszintű pásztázás kimerült" és „az összevont eltolás 17 találata hamis pozitív" állítás **téves volt**. A gyűjtemény a gazdaobjektum **`+0xf20`** eltolásán ül — kimondja a konstruktor egyetlen hívója (`0x00415a9f` `lea esi,[ebp+0xf20]`, `0x00415aa6` `call 0x4127c0`) —, tehát a `facerect` a gazdához képest **`+0x1ce0`**, és mind a 17 találat **valódi**. ⭐ **A HÁROM ÁG** (`FUN_00446610`): `facerect == 1` → **nulla téglalap + `0xF4240` (1 000 000) visszatérési kód** (`0x004466b4`, `0x004466eb`); `facerect == 0` → **tartalék út** (`FUN_00448270`); minden más → **`rect64` kicsomagolás** (`0x004467b2`), majd a `width` (`+0x13b0`) és `height` (`+0x1410`) oszloppal képpontra váltás (`FUN_009b93f0`). ⭐ **Ugyanez a küszöb harmadszor is:** a `FUN_00446370` soronkénti logikai tömbje szó szerint `facerect > 1`-et tesztel (`0x0044657c`–`0x00446585`). ⭐ **A `rect64` csomagolója `FUN_009b9150`:** `u64 = m0<<48 | m1<<32 | m2<<16 | m3` — a `0x004467b2` kicsomagolásának **bitre pontos inverze**. ⭐ **AZ ÍRÓ: `FUN_00480040`** — a tárolás `0x004810b3` (alsó 32 bit) és `0x004810bc` (felső), előtte azonosság-ellenőrzés, utána változás-értesítés (`0x006a2a60`); az érték útja téglalap → `FUN_009b9290` → `FUN_009b9150`. ⛳ **A belépési feltétel:** az író **csak NULLA** jelenlegi értékre ír (`0x00480de7`–`0x00480df1`) ⇒ **a meglévő téglalapot és az `1`-es jelzőt sosem írja felül** — ez magyarázza a mért 629 valódi / 2 575 × `1` / 1 140 × `0` megoszlást. ⭐ **A `CColumn` adattárolása is kiolvasva:** `+0x58` adattömb, azon belül `+0x4c` a sorszám **kétszerese**, `+0x48` az elemtömb; lépésköz **8** bájt u64-nél, **4** u32-nél; beágyazott alapérték `+0x60` (u64) / `+0x5c` (u32). ⛔ **Marad nyitva:** hol íródik konkrétan az `1` — a csomagoló nullát ad nulla téglalapból, és a `mov [reg],1` + `mov [reg+4],0` bájtmintára **nulla** találat van; hét `+0x1ce0` hely nincs végigolvasva. Jegy: **#2515**.

⭐ **2026-09-06 (7. kör) — az `imagedata` rekord ELTOLÁS-TÁBLÁJA, a `facerect` C++ TÍPUSA, és két helyesbítés (#1238, `picasa-imagedata-rekord.md`).** ⛔ **HELYESBÍTÉS:** a lap eddig azt írta, hogy a `0x004127c0` a `facerect`/`facerectdata` **írója** és hogy a rekord **38** mezős. Mindkettő téves: a függvény a gyűjtemény **konstruktora** (mezőnként egy `CColumn` a névvel és a típussal; a párja a destruktor, `0x00413020`), **értéket nem ír**, a mezők száma pedig **37** — tételesen kiolvasva. ⭐ **Új: a teljes eltolás-tábla mind a 37 mezőre** (`parent` `0x016c` … `backuphash` `0x11f0`), és a `pmp-database.md` három ismert eltolása (`width` `0x490`, `facerect` `0xdc0`, `facerectdata` `0xee8`) **betű szerint egyezik**. ⭐ **A `facerect` C++ típusa RTTI-ből:** `CColumn<unsigned___int64,1,322043908>` (vtábla `0x00c8230c`, ctor `0x00495360`) — és a harmadik sablonparaméter hexában **`0x13320004`**; a szomszédokkal együtt (`ytString` `0x13320000`, `unsigned_long` `0x13320001`, `double` `0x13320002`) az alsó bájt **pontosan a `.pmp` típuskód** ⇒ a #2105 típustáblájának **független megerősítése a C++ típusnévből**. ⭐ **Oszloponként SAJÁT zár:** a `CColumn` `+0x0c` jelző, `+0x20` szál-azonosító, `+0x24` rekurziószámláló, `+0x28` `CRITICAL_SECTION` (`0x007e3908`–`0x007e394d`). ⭐ **A Tulajdonságok párbeszéd (`0x007e3210`) nyolc oszlopot olvas** (`personalbumid`, `suggestionpersonalbumid`, `facequality`, `personalbumrecs`, `personalbumrecvalues`, `facerectdata`, `width`, `height`) — ⛔ **a `facerect`, a `deferredface` és a `deferredregion` NINCS köztük.** ⛔ ~~**A `facerect` ÍRÓJA NYITVA marad, öt kizárt úttal**~~ ⚠️ **ELAVULT JELÖLÉS (2026-09-09, 242. kör):** a `picasa-imagedata-rekord.md` „✅ A `facerect` ÍRÓJA és a `0`/`1`/`rect64` háromfelé ágazás" szakasza megválaszolta, és a **#2515 LEZÁRVA** (a Mappakezelő arcfelismerés-kizárása írja, `FUN_00446960` ← `FUN_00491210`). *(Az alábbi módszertani lelet — hogy az eltolás-pásztázás ebben a rekordban érvénytelen — továbbra is érvényes.)* Az eredeti szöveg: — köztük a legfontosabb módszertani lelet: az eltolás-pásztázás ebben a rekordban **érvénytelen**, mert a bázis `[obj+0xc0] + 0xf20`, a fordító a konstansokat összevonja; kontrollként öt biztosan írt oszlop (`edited`, `revertable`, `edit_width`, `edit_height`, `textactive`) is **nulla** találatot adott. ⇒ innentől dekompilálás kell. Jegy: **#2515** (a #1238 lezárva, ezért új jegy).

⭐ **2026-09-06 (6. kör) — a UI-lefedettségi axis KUTATÓI kínálata NULLA, és a 4./5. kör állítása MEGDŐLT.** ⛔ **HELYESBÍTÉS:** a 4. kör index-sora azt írta, hogy „**a rangsor száma a szerkezeti elemeket is beszámolja**", az 5. kör pedig „30 **feltáratlan** elemet" a `publish`-nál. **Mindkettő hamis.** A `ui-lefedettseg.md` fogalmi szakasza szerint a rangsor száma `feltáratlan + lekutatva + bizonytalan`, **csak az értékelhető elemekre**; a `rajzoló` osztály (a leltár **1284** eleme) `nem-ertekelheto`, tehát **eleve kimarad**. A két mérés nem ugyanazt méri: az enyém azt, hogy **említi-e a spec**, a rangsoré azt, hogy **megépítettük-e**. ⭐ **A mai öt vezető panel feltáratlan oszlopa** (`ui-lefedettseg.md:76–80`): makemoviepanel **0**, publish **0**, editpanel **0**, printoptions **0**, thumbui **2** — a 49/30/25/22/21 szám túlnyomórészt **`lekutatva`**, azaz **fejlesztői**, nem kutatói hiány. ⭐ **A maradék kettő is ARTEFAKTUM:** a `thumbui/single_action_message` egy egész lapon le van írva (`getmore-klipgyujto-mod.md` 3.2 — 335 × 26, `m_offsetL`, `Property textalign right`), csak a tábla a **levélnevet** használta horgony nélküli szakaszban ⇒ az osztályozó nem találta; a `thumbui/hviewtoggle` pedig **három** horgonyzott szakaszban szerepel (`picasa-fo-ablak-elrendezes.md` 4., `ui-audit-mainwindow.md` 1.4), tehát a 2026-09-04-i mérés **elavult** rá. ⭐ **Mindkettő javítva** (teljes nevek + `thumbui.tre` horgonyok), a szimulált osztályozó most 1, illetve 4 megfelelő szakaszt talál ⇒ **mindkettő `lekutatva`**. ⇒ **A UI-lefedettségi axisból kutatói kör már nem tud témát venni**; a jegylista és a `nyitott-kerdesek-sor.md` az egyetlen forrás. Jegy: **#2504** — a premisszája megdőlt, ezért átírva: a mérést **újra kell futtatni** (két elavult `hianyzik` tétel), és az osztályozó ne bukjon el a levélnevű táblákon.

⭐ **2026-09-06 (5. kör) — a `publish` panel ELEMLELTÁRA: 30-ból 28 valódi, de 7 online (`ajandek-cd-kimenet.md` 13.).** A `publish.tre` **106** elemnevet deklarál; a specek **47**-et említenek teljes néven, **59**-et nem — ebből **31 szerkezeti**, marad **28**. A 28 négy csoportba esik: `presentation_group` (Ajándék-CD, 11), `backup_group` (mentés, 3), `replication_group` (feltöltés, 7) és a **`m_hidden` `web_group`** (webre publikálás, 7). ⛔ **A `web_group` hét eleme online funkció ⇒ NEM építendő** — a leltárban azért van benne, hogy a mérő ne számolja hiánynak. ⭐ **11 felirat** angolul és **hivatalos magyarul** is megvan (`publish_text.tre` + `panel-feliratok-hu.tsv`, soronkénti horgonnyal): pl. `selectiontext` = „Kijelölés és beállítások", `cdnametext` = „Az ajándék CD elnevezése", `namelimitext` = „Legfeljebb 16 karakter", `uploadsync3` = „Szinkronizálás:". ⭐ **Geometria mind a 28-ra** a `respack.yt` rétegfejléceiből (vászon 1024 × 212); a két lépés-keret azonos méretű (**311 × 166**), a mentés-üzemmód `backuprect2`-je ugyanoda, szélesebben (**324**) kerül ⇒ a panel **lépésekre osztott**, az üzemmód a keretek tartalmát cseréli. Jegy: **#2508** (a 21 építendő elem).

⭐ **2026-09-06 (4. kör) — a `makemoviepanel` „49 feltáratlan eleme" valójában HÁROM (`picasa-create-features.md` 2.11).** A `makemoviepanel.tre` **102** elemnevet deklarál; a `docs/specs/` **60**-at említ teljes néven, **42**-t nem. A 42 osztályozva a `.tre` szülőviszonyából: **12 ikon-gyerek** (egy már dokumentált gomb része), **15 konténer**, **4 csúszka-tok**, **4 elválasztó**, **4 kezdetben rejtett** — és **3 VALÓDI**: `albumname` (felirat az 1. fülön, szövegét a `0x0061a6c0` és a `0x0061bbd0` állítja), `indicator` (a filmszalag **lejátszásjelzője**: `m_centerX` a `filmstrip`-en, a kezelő `FUN_006214e0` a téglalapja közepét számolja, `0x00621684`–`0x006216c1`), `audiostrip` (hangsáv-csík). ⛔ **NEGATÍV:** az `audiostrip`/`audioclip` nevére **nincs** kódhivatkozás — szemben a `filmstrip`, `indicator`, `previewimage`, `backcolor`, `show_captions` nevekkel. ⇒ **a rangsor száma a szerkezeti elemeket is beszámolja**; a panelre a tényleges hiány 3 elem. Jegyek: **#2503** (a három elem), **#2504** (a lefedettségi mérő szerkezeti-elem szűrése).

⭐ **2026-09-06 (3. kör) — a `.cxf` `scale` LÁNCA ÖSSZEÁLLT, és egy ELLENTMONDÁS maradt (#1412, `kollazs-eletciklus.md` 17.13–17.14).** ⭐ **A hiányzó láncszem megvan:** a `+0x64`/`+0x68` staging-mezők **olvasója** a kollázs-sávban pontosan egy függvény — `FUN_00833920` (`0x008339aa`, `0x008339b7`) —, és az a csomópont-tömb **`push_back`**-je (`mul 0x38` a `0x00833a92`-n, elemmásolás `0x008341b0`-nal, az új elem címe `lea eax,[edx+ecx*8]` a `0x00833b4c`-en). ⇒ **Teljes lánc:** fájl → beolvasó → staging (`+0x64`/`+0x68`) → `push_back` → tömbelem (`+0x28`/`+0x2c`) → `.cxf`-író. ⛔ **Amit ez kimond:** a `scale` a `Picasa3.exe`-ben **csak két helyről** kaphat értéket — a téma-elrendezőktől (állandó **1,0**) vagy a **fájlból**; **számoló írót egyik pásztázás sem talált**. ⚠️ **Az ellentmondás:** a mintáinkban 313/330 áll. Vagy (a) a fájlok értéke korábbi mentésből származik, vagy (b) van egy **mutatón át** író (10 `lea r,[r+0x2c]` hely). **A döntő olcsó mérés: egy ÚJONNAN létrehozott kollázs `.cxf`-je** — ha ott `scale="1.000000"`, az (a) igazolt. Jegy: **#1412** (`felhasználóra-vár`).

⭐ **2026-09-06 (2. kör) — a `.cxf` `scale`: a SÁVON KÍVÜLI keresés is NEGATÍV, és megvan a hiányzó láncszem HELYE (#1412, `kollazs-eletciklus.md` 17.10–17.12).** ⭐ **Csomópont-alakú float-írók az EGÉSZ binárisban** (`+0x2c` ÉS `+0x28` float, plusz legalább három mező a `+0x18`…`+0x2c` sávból, esp/ebp kizárva): **16**, ebből 10 a sávon kívül — az egyetlen valódi jelölt (`0x00819f50`, mert a kollázs-sávból hívja a `0x00873cb0`) **három (x,y) pont**, nem `theta`+`scale`. ⭐ **A burkoló `+0x68`-as résére** az egész binárisban **öt** float-író van, a kollázs-sávban **csak a beolvasó**; a `0x0082fab0` utasításhatáron belüli bájtminta-találat. ⛔ **HELYESBÍTÉS:** az író csomópont-tömbje a **2. argumentum**, nem az első — a `0x00985ff0` **`ret 4`**-gyel zár (`0x0098601d`), tehát a `0x008350ae`-nél `[esp+0x24]` = arg2; a bájteltolást (`[ebx+0x48]`) az író **tízszer olvassa és sosem írja**, a hívó állítja (`0x00834777`). ⭐ **A hiányzó láncszem HELYE:** a beolvasó `+0x64`/`+0x68`-ba ír, a tömbelem viszont `+0x28`/`+0x2c` ⇒ van egy **burkoló → tömbelem** átvitel, amit még senki nem azonosított; a következő lépés a `+0x64`/`+0x68` **olvasóinak** pásztázása. A dokumentum-konstruktor `FUN_00832500` (`CCollageParser::vftable` `0x00cbf878`): `+0x3c = 2` (a `version="2"`), `+0x48 = 0`. Jegy: **#1412**.

⭐ **2026-09-06 — a szerkesztő NAGYÍTÁS-vezérlőinek kutatása MÁR KÉSZ VOLT (#2492, `ui-audit-editor.md`).** A jegy három bináris kérdést kért (hely · csúszka-leképezés · mit tesz a `fit`/`1:1`); mindhármat a **#2305** és a **#2312** már megválaszolta, és ezt a kör **függetlenül újramérte**: `fit` → `0.0f` (`0x005d5ccc` `fldz`), `1:1` → `0.5f` (`0x005d5d45`, `0x00c7dafc`), a visszajelző hármas `FUN_005d1c70`, a görbe pedig `0 ≤ v < 0,5`: `1 + (2^(2v) − 1)(r − 1)`, `0,5 ≤ v ≤ 1`: `r · 2^(4(v − 0,5))` (`FUN_00a5f500`, `pow` = `0x005568e0`) ⇒ **max. 400 %**. ⭐ **ÚJ ebben a körben:** a **léptető** út (`FUN_005d1300`) `[0, 1]`-re **vág** (`0x005d13a9` `fldz`, `0x005d13c6` `fld1`) és a **0,5-nél BEAKAD** (`0x005d137b` + `0x005d13e3`–`0x005d1429`), ráadásul **nem animál** (`push 0`), szemben a gombokkal (`push 1`). ⭐ **A tulajdonosi képernyőmentés kimérve** (1920 × 1200): az eredetiben a hármas az **alsó eszközsávban**, közvetlenül a forgatás-gombok után ül — egybevág a mért `m_offsetRB` horgonyzással (440 px a jobb széltől, 50 px a talp fölött). ⛔ **Nálunk (MÉRVE):** a `zoomBar` a kép alatti külön sávban van (`PhotoViewer.qml:1610`), és a csúszka `from: 0.25 to: 8` a NYERS szorzón (`:1713`) — nem a normalizált `[0, 1]`-en, ezért nincs 0,5-ös 1:1-horgony. ⇒ **a #2492 nem kutatói, hanem fejlesztői feladat** (a `bináris-kutatható` levéve).

⭐ **2026-09-06 — a `.cxf` `scale`-jét a KOLLÁZS-SÁVBAN SENKI NEM SZÁMOLJA (#1412, `kollazs-eletciklus.md` 17.5–17.10, `picasa-create-features.md` 1.6/g).** ⛔ **MEGDŐLT** az előző kör kimenete („a végleges értéket a kollázs-sáv 37 mutatós `+0x2c`-írójának egyike adja"): a `+0x2c` eltolású írások túlnyomó többsége **`[esp+0x2c]` lokális változó**. Helyes ModRM/SIB-szűrővel (SIB `base != 100b`, mutatósnál `rm ∉ {100b,101b}`) a sávban **47** valódi író van, és **float**-ot csak **hat**: `0x00822230`/`0x00823620` (általános geometria-segéd, nem a csomópont), `0x008341b0` (a csomópont `operator=`), `0x0087b830` (másolás), és a két elrendező — `0x00885060` (`regulargrid`), `0x00888210` (`contactsheet`) —, **mindkettő `fld1` ⇒ állandó 1,0**. ⭐ **Új, harmadik forrás: a BEOLVASÓ** (`0x00832830`) közvetlenül a csomópontba ír (`0x008332b7` `fstp [ebx+0x68]` = `scale`, `0x00833240` = `theta`; a burkolóban a csomópont a `+0x3c`-nél kezdődik). ⛳ **Pozitív kontroll:** a `multiexp` (`AI7.cxf`) `scale="1.000000"` — pontosan az elrendező `fld1`-je, tehát az „elrendező → fájl" út egy témán végig igazolt. ⭐ **Mérés a korpuszon:** a 12 minta **97** `scale` értékéből **95 egész**, és a `picturepile` létrája (238·249·263·280·303·337) **betű szerint ugyanaz** hat független kollázsban; a két tört érték (`AI2`) a kézi átméretezésé. ⭐ **Az `AI6` vízszintes rácsa pontos egész:** bal margó **90**, oszlop-osztás **300**, cella **242**, és a keskeny kép **vízszintesen középre** kerül (`(242−155)/2 = 43,5` → **43**), függőlegesen viszont **felülre**. ⛔ ~~**A kérdés NYITVA marad**, de a keresés helye megváltozott: a sávon KÍVÜL, vagy mutatós írás (10 `lea r,[r+0x2c]` hely a sávban).~~ ⚠️ **ELAVULT JELÖLÉS (2026-09-09, #2805):** a mutatós út is negatív, és az indexelt (SIB) alak is — a gépi keresés **három címzési alakra** megerősítve kimerült; a maradék döntést egy friss `.cxf` hozza. Jegy: **#1412** (`ready` + `bináris-kutatható`).

⭐ **2026-09-05 — a `text=` 8. MEZŐJE MEGFEJTVE, kontrollált minta NÉLKÜL (#2108, `picasa-ini-format.md`).** ⭐ **Bit 8–15 = vízszintes igazítás:** a kattintáskezelő (`0x0062e8f0`) a három elemnévre (`leftalign` `0x00c9e8f4` · `centeralign` `0x00c9e90c` · `rightalign` `0x00c9e928`) rendre **0 / 1 / 2**-t tesz a panel `+0x2e4`-ébe (`0x0062efbf`, `0x0062f085`, `0x0062f1a6`), majd minden kijelölt feliratra meghívja a `[vtbl+0x2c]` beállítót — épp azt, amelyik a 8. mező 8–15. bitjét kapja. Ezzel a **bal** igazítás is megvan, amire a korpuszban nem volt példa. ⭐ **Bit 0–7 = kitöltés/körvonal mód, és SZÁMÍTOTT, nem tárolt:** a panel az EGYETLEN `ytTextSettings`-leszármazott az egész binárisban (a `.text` kimerítő pásztázása szerint), és a getter-felülírása (`0x00630450`) minden hívásnál újraszámol: `no_fill` bepipálva → **1**; különben a körvonal-csúszka pontosan 0,0 → **0**; egyébként → **2**. A beállító-felülírás (`0x006303d0`) visszafelé is karbantartja (pipa `érték==1`-re, csúszka nullázása `érték==0`-ra). ⛳ **A korpuszon 5/5**, mindkét irányban pozitív esettel: a 4. blokk 0,250-es körvonala → 2, az 5. blokk 0,500-as körvonala mellett is **1**, mert a `no_fill` ága előbb fut — és a kép ott tényleg csak körvonalas. ✅ **A FELSŐ 16 BIT IS LEZÁRVA (ugyanaznap, második kör):** a panel **minden szerkesztéskor nullázza**. A `ytTextSettings` konstruktorát az egész binárisban **két** hely hívja (`0x005bb758`, `0x00a4da07`), és mindkettőt másolás követi; a másoló (`0x00a4e9e0`) tulajdonságonként getter→setter párokkal dolgozik, a felső 16 bitet a `[src+0x48]` → `[dst+0x38]` pár viszi (`0x00a4ea88`→`0x00a4ea91`); a panel pedig a SAJÁT bázisát (`+0x28c`) másolja minden kijelölt feliratba (`0x00630c70`, a `0x0062e4ac`-ról), és a `[vtbl+0x48]` slotja a `0x004bdeb0` = `xor al,al; ret` konstans nulla ⇒ a konstruktor 1-ese felülíródik. Ez magyarázza a korpusz **7/7** nulláját, és kimondja: **nem érintetlen mező, hanem aktívan nullázott**. ⛔ **Negatív lelet a jelentésről:** az osztály mind a 40 vtábla-metódusában a `+0x14`-re pontosan két hivatkozás van (a saját beállítója és gettere), a panelnek nincs rá vezérlője, és a teljes `edittextpanel`-tartományban (52 függvény) egyetlen `[vtbl+0x48]` slot-hívás sincs ⇒ a felirat-funkción belül **nincs fogyasztója**. A hatókör kimondva: ezen kívüli fogyasztót nem lehetett kizárni (a `[vtbl+0x48]` alakra 279 függvény jelölt, nem szelektív). ⛔ **Nálunk (MÉRVE):** az `ini/text_overlay.py:166` a 8. mezőt `unknown_b: int = 0` néven őrzi meg értelmezés nélkül; a rajzoló (`render/text_overlay.py`) **tudja** az igazítást és a `fill_enabled`-et, de az `edit_controller.py:447/1024/1091` szerint mindkettő csak munkamenet-állapot, mentés nélkül. Jegy: **#2108**.

⭐ **2026-09-04 — a `text=` stílusblokk MÉRTÉKEGYSÉG-LEKÉPEZÉSE (#2271, `picasa-ini-format.md`).** ⭐ **A tagtérkép TELJES:** az előző kör a beolvasóból négy mezőt kötött taghoz, az **író** (`0x00a4e3b0`) getterei a maradék ötöt is megadják — 1. `+0x20`, 2. `+0x38`, **5. `+0x3c` (float)**, **6. `+0x34` (float)**, 7. `+0x1c`, 8. három bájt (`+0x14`, `+0x2c`, `+0x28`), 9. `+0x18`. ⭐ **A két csúszka útja: NINCS átszámítás.** A `0x0062f3c0` kezelő az átlátszatlanság-csúszka nyers értékét teszi a `+0x34`-be, **alulról 0,1-re vágva** (`0x0062f460`, a konstans `0xcf4888`/`0xc7e4a0` = **0.1**) ⇒ nulla átlátszatlanság nem áll elő; a körvonalvastagságot ugyanígy a `+0x3c`-be, **de a pontos 0,0 nem tárolódik**: a program a „nincs körvonal" ágra ugrik (`0x0062f555` → `0x0062f581`) — ez a `no_fill` négyzet pipáját veszi le; **a 8. mező alsó bájtja nem tárolt, hanem SZÁMÍTOTT** (helyesbítve 2026-09-05, #2108). ⭐ **A méret képlete:** ha a 9. mező `0x8000` bitje áll (minden mintában áll), a tárolt érték = **em-képpont ÷ a kép MAGASSÁGA** (`0x00a4e5aa`); ha nem áll, `× 360,0` is (`0xcf3d50`, kiolvasva). A korábbi képpont-mérés ezt függetlenül igazolja (82,13 és 140,62 a mért 82/141 mellett). ⭐ **A csúszka TARTOMÁNYA is megvan: `[0, 1]`** — a `ytSliderHandler` (RTTI `0x00cda46c`) érték-beállítója (`vtbl+0x10` → `0x00aaf220`) a kattintás helyét a **sáv hosszával** osztja (`0x00aaf252`), majd **0,0-ra és 1,0-ra vágja** (`0x00aaf259`–`0x00aaf282`); a görgő egy kattanása a tartomány **2 %-a** (`0xcf48a8` = 0.02, a delta ÷ 15,0). ⇒ a mintákban látott 0,25 és 0,5 a csúszka negyed, illetve fél állása, és **nálunk a 0–8-as, egész lépésű körvonal-csúszka MÉRTÉKEGYSÉGE az eltérés**, nem a leképezés. ⛔ **Szűkült, nem oldódott meg:** a 0,757813 tárolt vs. 0,53 mért átlátszatlanság eltérése **nem az írásban van** (az azonosság), hanem a RAJZOLÓBAN. Jegy: **#2271** (kommentelve, `ready` vissza).
...
### [filterdesc-registry.md](filterdesc-registry.md) — 1 NYITOTT tétel (⭐ **2026-09-23 — a DropShadow-elmosás SIMD-ágai (amelyek minden SSE2-es gépen futnak: `0x00c33d30`, CPUID EDX 26) bájtra azonosak a skalár úttal: a vektoros út `_controlfp(_RC_DOWN)`-nal lefelé kerekítésre állítja az MXCSR-t (`0x009025c0`, `0x00d333a4`); emulátorral 39 × 3 esetben 0 eltérés (#626).** ⭐ **2026-09-22 — a `DropShadow` `shadowAlpha` útja utasításszinten (#626): `[op+0x24]` → float32 → `clamp(0,1)` → rekord `+0x0c` → a rajzoló csonkolásra állított `fistp`-vel `TRUNC(·255)`-öt ír (`0x00bcda7a`); a spec és a kódunk `ROUND`-ot írt — 48 egész Fade-értéknél 1 szinttel eltér, fejlesztés #3498.** ⭐ **2026-09-22 — a `DropShadow` `quality=3` natív blur-menete feltárva (#626): 3 vízszintes + 3 függőleges, egymásra épülő integer/futóablakos menet; a PicasaPy-ba átvezetve és emulátorral bitre igazolva (#3474; a 684-es goldenen ΔE 0,713 → 0,084).** ⭐ **#626, 2026-09-19: a Border iránykérdése és a `SimpleColorMatrix` hue-mátrixa lezárva; a TwoTone golden a natív első RGB-csatorna erős kontrollját adja, a `0x008f2640` mátrix-alkalmazó és a `0x008f2040` linked mátrixépítő CSATORNA-SZIMMETRIKUSNAK bizonyult; ⭐ **a `TwoTone` gradiens-skalára MEGVAN (`0x00bb87b0`+`0x00bcb2f0`): a LUT négy 256-elemű rekeszéből csak a `+0x800` (a `src[2]`, azaz a piros csatorna bájtja) töltődik fel, a másik két szín-rekesz NULLÁZOTT — a `TwoTone` tehát NEM lumát, hanem a nyers piros csatornát vetíti a gradiensbe; a `glimmer_tone.py` Rec.601-luma útja bizonyítottan téves modell, fejlesztői jegy nyitva;** ⭐ **A #2746 LEZÁRVA (2026-09-12, #2770): a kérdés második fele volt a hibás premissza.** A tulajdonos leszállította a kért második, természetes átmenetes képet, és azon a Picasa kimenete **nem ül egyenletes rácson**: a `Steps = 8`-as rácshoz az illeszkedés 11,94% (az első, mértani mezős képen 97,62% volt), a lapos területeken is csak 13,36%, tehát utólagos simítás sem mosta el. Ami helyette van: **hét színű, KÉPFÜGGŐ paletta** — a lapos területek 97,92%-át 7 szín fedi, a csatorna-szintek nem egyenletesek, és a hetes szám egybeesik a bináris kiolvasott palettaméretével (`Steps−1` = 7). A rácsos olvasat tehát az első mérőkép sajátja volt, nem a szűrőé; a mai rácsos implementációnk természetes fotón mérhetően más képet ad. Részletek: `filterdesc-registry.md` **1/b**. A #2125 korábbi jelölése a GitHubon **lezárult**; az `Invert`-jelvény része nem marad külön nyitott kutatási tételként. A `filterdesc.xml`-kérés 2026-09-08-án FELOLDVA: a telepítés-mentésben megvan, bájtra azonos a miénkkel. A #2456 élő csempe-útja LEZÁRVA; a megvalósítás külön fejlesztői jegy: #3315)

⭐ **2026-09-05 — a Comicize pontmaszkja STATIKUS: a tónus nem a maszkból jön (#2476).** A `filterdesc.xml` két `TiledImageMask`-ja (`_mskColorSpots1/2`) **semmilyen képfüggő attribútumot nem kap** — a tónus a láncból jön (`AdjustCurves` → `Pixelate` → `BW` → küszöbgörbe → `GetVar … Mask`). ⚠️ **Nálunk fordítva:** az `apply_comicize` a `halftone_branch`-et hívja, ami a pont SUGARÁT modulálja a tónussal (`halftone.py:82`), miközben a `TiledImageMask` tényleges megfelelője (`tiled_dot_mask`, `halftone.py:94`) **nincs használatban** ezen az úton. **A rajzoló primitív kimérve:** a csempe-előállító (`0x00bbaa90`) ugyanazt a párost hívja (`0x008f3840` + `0x008f3970`), mint a `CircularGradientImageMask` (`0x00bc2a50`), **ugyanazzal a harmadik argumentummal (2)** — és az nem alakzat, hanem **megállószám** (a `0x008f3970` bájttömbként olvassa, `[ebp+0xc]−1` hosszú cikluson; a harmadik hívó 15-öt ad). ⇒ kétmegállós színátmenet, nem kemény küszöb. **MÉRVE** (fázis-hajtogatás a 24-es csemperácsra, `comicize-sweep/…/Comicize_050.jpg` + a Picasa exportja): a referencia profilja **fennsíkos** (1,00/0,93/0,82), majd `r=4`→0,40 és `r=6`-ra nulla; a miénk fennsík nélkül monoton lejt, `r≈7`-ig elhúzódó farokkal. Csúcs-csúcs **13,20 vs 25,89 (1,96×)**, csempénkénti amplitúdó **26,2 vs 36,4 (1,39×)** — ez erősíti meg a #1606 „kb. 1,5×"-ét. Jegy: **#2476** (`ready`, a javítás fejlesztés). ⭐ **2026-09-12 — a pont PROFILJA megfejtve:** a csemperajzoló két megállót ad át (`0x00bbacba push 2`), pozíció `0x00`/`0xFF`, alfa `CSONK(alphaMax·255)` a közép és `CSONK(alphaMin·255)` a szél — tehát **lineáris radiális rámpa**, küszöb és felül-mintavételezés nélkül; a középpont a `0,5` szorzóból (`0xc72150`). Az attribútum-eltolásokat a csempe **gyorsítótár-kulcsa** igazolja (`"%d-%d-%d-%d-%d-%d-%g-%g-%g-%g"`, `0x00bba9d4`), és ugyanez a kulcs második, független bizonyíték a maszk képfüggetlenségére. ⚠️ A mért fennsíkos profil a TELJES lánc kimenete, nem a maszké — a kettő nem közvetlenül összevethető. ✅ **2026-09-18: az `alphaMax`/`padding*` tartalékértékei a konstruktorból megvannak** (`alphaMax=1,0`, mind a négy `padding=0`). Részletek: `filters-decoded.md`.

⭐ **2026-09-19 — #3390 LEZÁRVA: a Comicize kétmegállós pont-LUT bájt- és képponti útja megvan.** A `0x008f3700` csatornánként `TRUNC(c0 + t·(c1−c0))`-t ír, a két alfa-végponttal a 256 rekesz pontosan `[255,254,…,1,0]`; a `0x008f3970` a sugár 8.8-as LUT-rekesz + törtbyte párral kever, `sqrtps`-sel négy képpontot dolgoz fel párhuzamosan, **nem** végez felülmintavételezést. A mai kontroll: 82 célzott próba zöld, `dot_size_for(1600)=24`, `DOT_SCALE=0,8`, `_EDGE_SOFTNESS_PX=1,0`; utóbbi külön, binárissal nem igazolt terméki ág. A kutatási mechanizmus megerősített, a PicasaPy-átvezetés és a 15 exportos hatásmérés külön fejlesztői munka: **#3401**. Részletek: `filters-decoded.md` Comicize-szakasza, #3390.

⭐ **2026-09-05 — a szerkesztőnek van egy HATODIK, rejtett füle: a VIDEÓ-fül, és az elrejti a fülsávot.** Az `editpanel` panel hat *bizonytalan* eleme mind lezárva. A `movietab` (`editpanel.tre:476`) nem hatodik fül a sorban: a `showtarget movietabpanel` mellett `hidetarget editpanel/tabs` és `fxthumbs` is áll ⇒ bekapcsolva **eltünteti a teljes fülsávot és az effekt-csempéket** — a szerkesztő másik üzemmódja. *(Az eredetiben egyébként ÖT fül van: `tab1`…`tab5`.)* **Be:** `0x00593dcf` → `0x9cd8a0("editpanel/movietab")`; **vissza:** `0x005f81da` → ugyanaz `"editpanel/tab1"`-gyel, és csak akkor, ha a `movietab` nem aktív (`byte [eax+0x359]`, `0x005f81ca`). A sztringre az EGÉSZ binárisban pontosan két függvény hivatkozik. **A panel tartalma** a `0x00593880`-ból: `video_control_bar/{setin,setout,lefttrim,righttrim,trimslider,startthumb,endthumb,scaleslider,volumeslider,time,controlbar}`, `moviecontrols/{play,pause}`, `movieeditpanel/{reset_trim,capture_frame,export_movie,export_youtube}`, `editpanel/{render_now,in_progress_label}` + az `AutoPlayMovies` beállítás ⇒ a trimmelés és a képkockamentés ehhez a fülhöz tartozik (**#1838**). A másik négy: `modaldialogblur` = elhomályosító réteg a modális párbeszédek mögött (`:1362`), `picnikapply` = a Kreatív készlet „Alkalmaz” gombja (`:1430`, megszűnt szolgáltatás), `preview2` = a **kettős nézet** második előnézete (`:1066`), `editpanel` = konténer. Lap: `ui-audit-editor.md`; a hat elem a `ui-lefedettseg-elemek.csv`-be is bekerült.
...
⭐ **2026-09-23 — a Comicize ágának két lépése kiolvasva (#3507):** a `BWImageOperation` folytonos Haeberli-szürke színmátrix (`0x00bbdd80`: 0,3086 / 0,6094 / 0,0820), nem küszöböl; a maszkos `GetVar` `GetVarInstruction` → `PartialMask` → `Pop`-ra fordul (`0x00bc4ae0`), `⌊(g·m + 255·(255−m))/255⌋`. A rasztervesztés oka máshol van. ~~**Nyitott kérdés:** … #3511.~~ → **LEZÁRVA (2026-09-23, #3511):** a kísérletből a `_opColorSpots` nyitó fekete Glow-ja hiányzott (`filterdesc.xml` 788); vele a szó szerinti lánc a 15 exporton az amplitúdó-hibát 0,0276-ra hozza (ma 1,4331). Fejlesztés: #3522.
...
### [picasa-create-features.md](picasa-create-features.md) — 1 NYITOTT tétel (a #1412: **honnan kapja a csomópont a `+0x2c`-t (`scale`)?** ⭐ **2026-09-10, 266. kör: a tömb FELTÖLTŐJE MEGVAN** — `FUN_00881710` egy 16 bájtos `bal/fent/jobb/lent` téglalap-tömbből tölti a `[dok+0x48]`-at (x, y, w = jobb−bal, h = lent−fent), 56 bájtos lépésközzel, és a `+0x2c`-hez meg a `+0x28`-hoz **hozzá sem nyúl**. ⭐ **265. kör:** a `picturepile` `scale`-képlete megvan és 9/9 pontos — `TRUNC(1024 × 0,33 × min(1/√(√k − 1), 1))`, író `FUN_00834520` @ `0x00834683`, témára kapuzva. ⛔ `contactsheet`-kapu **NINCS** (a hat téma-sztring minden hivatkozása átnézve). ⇒ **A `scale` a LAYOUT ELŐTT dől el**; a maradék kérdés a csomópont-HOZZÁFŰZŐ út, horgonya: az alap-ktor (`FUN_00833cd0`) sem a `+0x2c`-t, sem a `+0x28`-at nem inicializálja, tehát a hozzáfűzőnek MINDKETTŐT írnia kell. `kollazs-eletciklus.md` **55.** és **56.**)

⭐ **2026-09-05 — a `.cxf` `scale` ÍRÓJA megvan, és semmit nem alakít át (#1412).** A lap eddig a `+0x2c` mezőt csak a **layout** oldaláról azonosította a `scale`-lel; most az **író** oldaláról is megvan, tehát a mezőazonosság **két független forrásból** áll. `FUN_008347b0` (ugyanaz, amelyik az `albumTitle`/`orientation`/`shadows`/`theta` attribútumokat is írja): `0x00835096` `push "scale"` (`0x00cbf80c`) → `0x008350ab` `mov ecx,[ebx+0x48]` → `0x008350ae` `mov edx,[esp+0x24]` → **`0x008350b2` `fld dword ptr [edx+ecx+0x2c]`** → `%f` (`0x00c817c0`) → `0x008350c9`. ⛳ **Kontroll a valódi fájlon:** a `%f` **hat tizedest** ad, és a mintáink pontosan így néznek ki (`scale="337.000000"`, `scale="1.000000"`) ⇒ nem másik függvény írja. ⇒ **a fájlban álló `313` pontosan a csomópont `+0x2c` mezője a mentés pillanatában**, íráskori átszámítás nincs. ⚠️ **Ez a kör NEM vezette le a 313-at.** ⛔ **A pásztázás-állítása 2026-09-06-án MEGDŐLT** (ld. a fenti napi bejegyzést): a `+0x2c` eltolású írások többsége `[esp+0x2c]` lokális, és a helyes szűrővel a sávban **hat** float-író van, köztük a két elrendező **állandó 1,0**-val, plusz a **beolvasó** (`0x008332b7`). A **#1412** ezért `ready` + `bináris-kutatható`: a keresés a sávon KÍVÜL, illetve a mutatós írásoknál folytatódik; a fekvő Indexkép-minta már csak független megerősítés.
...
1712 olvasója `cmp …, 0` null-ellenőrzés. ✅ **LEZÁRVA 2026-09-06 (11. kör):
a gomb működése MEGVAN** — `picasa-create-features.md` 2.12; a „az olcsó lánc
kimerült" megjegyzés téves volt (a kör a MINŐSÍTETT nevet kereste, az elosztó
a PUSZTÁT hasonlítja). A fülek magyarul: **„Mozgófilm" ·
...
3. **Az Indexkép (contactsheet) `.cxf` `scale` ÉRTÉKÉNEK képlete** — a
   `scale` SZEREPE 2026-09-06-án **lezárva** (`kollazs-eletciklus.md` **18.**):
   ...
   **lezárva**: az a cellaél `k`, **nem** a `scale` (19.1). A közvetlen írás
   ...
   55/57 — 20.1), és ezzel a `scale` egységes olvasata is: a csomópont
   **csempemérete lapegységben**. A `contactsheet` csempeméretének zárt
   képlete marad nyitva; a keresés helye a `CContactSheetTheme` saját
   csempeméret-írása, a `0x0082cad0` (`[téma+0x3c]`) analógiájára.
   ⭐ **2026-09-06 (171. kör):** a `scale` **mértékegysége bizonyítva** — hossz a
   lap szélességének 1024-ed részeiben (`FUN_0087c470`, a `0,0009765625`
   konstans, 21.2); és az Indexkép eleme a `scale`-t a rajzoláshoz **nem is
   használja** (21.3), ezért tisztán elrendezési mennyiség.
   ⭐ **2026-09-07 (172. kör):** a `+0x2c`-írás **öt címzési alakja** kizárva
   (a `lea`-út is, pozitív kontrollal — 22.1/22.6), és a hozzáadó
   `push_back` a `CCollageParser` **vtáblájának** 8. slotja, tehát csak a
   beolvasó hívja (22.4) ⇒ a keresés a **képhozzáadási** útra fordul.
   ⭐ **2026-09-07 (173. kör):** az **olcsó lánc KIMERÜLT** — a futásidejű
   eltolás és a blokk-másolás is kizárva (23.3, 23.4), a `FUN_0087c470`
   visszaírása utasításszinten cáfolva (23.2); ⛔ és kiderült, hogy a
   korábbi körök „kollázs-sávja" **túl tág volt** (az alja arcfelismerés,
   23.1). **A következő lépés a DRÁGA út:** célzott dekompiláció a
   `0x0082a670` (kollázspanel) **képhozzáadási** ágára.
   ⭐ **2026-09-07 (174. kör):** a **K2 olcsó lánca is a drága úthoz ér** —
   új összefüggés `k = min(cellaSzél, cellaMag)` (4/4, 24.1), az illesztés
   magja utasításszinten (24.2), és egy **kimondott kétértelműség** a
   `FUN_009b4aa0` két rectjének szerepéről (24.3), amit csak
   verem-nyilvántartás dönt el. ⇒ **egyetlen Ghidra-menet, két gyökér:**
   `0x0082a670` (K1) és `0x00888210` (K2).
   ⛔ **2026-09-07 (175. kör):** a felhős Ghidra-kör **nem indítható** — a
   tokenből hiányzik a `codespace` jogosultság (25.1); a keret NEM akadály.
   Helyben a **célkeret azonosítva** (a cella, 25.2), a **forrásrect nem**
   (25.3, a nyers `esp`-illesztés korlátja), és a mérés–kódolvasat
   **ellentmondása kimondva** (25.4). Üzemeltetési jegy: `picasapy-agent` #53.
   **#1412**, **#2583**, **#2593**.
...
⛔ **NYITOTT: MELYIK fájl bájtjait rögzíti az `originhash` a mentés pillanatában** — a lemezre most kiírt (szerkesztett) képét, vagy a szerkesztés előtti eredetiét? A kulcsnév („origin") és az `origloc`-párja az utóbbit sugallja, de **nincs bizonyítva**; a 60-as mintában 44 sor nem egyezett a MAI fájllal, és ezt sem az mtime, sem szerkesztési kulcs (`filters=`/`redo=`/`crop=`) jelenléte nem magyarázta. **Amíg ez nyitva van, a mi `originhash`-írásunk nem javítható** (ma SHA-256-ot ír a `redo=` láncra — se a hossz, se a bemenet nem stimmel). Ez a kérdés önálló jegyet érdemel: eldöntéséhez egy KONTROLLÁLT minta kell (ismert, szerkesztés előtti eredeti + a Picasával mentett kimenet együtt).

### [picasa-ini-format.md](picasa-ini-format.md) — 1 BLOKKOLT tétel (a `[encoding]` fejléc, #2452; a `text=` stílusblokk MIND A KILENC mezője 2026-09-05-én LEZÁRULT)

⭐ **2026-09-07 (#791) — az öt sűrű, „nem kezelt" kulcs round-tripje MÉRVE, és egy MEGKERÜLŐ ÍRÓ leleplezve.** A leltár öt kulcsot emelt ki (`IIDLIST_<fiók>_lh` 6 045 · `originhash` 1 787 · `P2category` 615 · `category` 179 · `geotag` 84). **Mérve mind a 859 valós fájlon:** a `parse_document`→`serialize` kör **0 eltérés**, és egy közbeiktatott írás után a **8 710 célkulcs-példányból 0 elveszett, 0 elváltozott** ⇒ az `ini/` kapun át a megőrzés hibátlan. ⛔ **DE egy hívó megkerülte a kaput:** a kollázs/film projekt-megjelölése (`app/collage_output.py`, `write_album_ini`) soralapon írt vissza, és a #1088 óta ez a VALÓDI Picasa-mappa. Három MÉRT kár egyetlen íráson: **11/11 CRLF→LF** (⇒ a fájl minden sora megváltozott), **U+FFFD-rontás** a régi, nem UTF-8 fájlok ékezetes feliratain (visszafordíthatatlan), és **második `[Picasa]` szekció** BOM-os fájlnál; `OSError`-ág esetén a teljes tartalom elveszett volna. Javítva: a megjelölés is az `ini/` API-n megy, a #1097 helyben-írását a `save_document(..., in_place=True)` viszi tovább. ⭐ **A `geotag` ticket-pontja ELAVULT volt:** a #30 óta olvassuk (`index.geotagged_photos`, `photos.geotag_ini`) ÉS a Helyek-panelnek átadjuk (`app/geo_controller.py`) — nem volt teendő. ⭐ **Az `originhash` MEGFEJTVE ugyanebben a körben:** **nem egy digest, hanem KETTŐ** — `fmt16(originfast) + fmt16(originslow)`, 16 + 16 kisbetűs hexa jegy. Ezért bukott korábban mind a nyolc egy-digestes jelölt (#1481 „0/32"). Bizonyíték: a **szétszedő** `0x00414b40` (`cmp eax,0x20`, majd 16+16 külön `sscanf "%I64x"`-szel), az **összerakó** `0x00414c50` (`%016I64x` ×2, `0xc80ce4`), a kiírás `0x007d5e74` (`0xcb9254`), és a két hasher `0xa4d210` (fej+farok, #1481) / `0xa4ce40` (teljes fájl, #1482); a felosztás irányát három független jel adja (`0x0070e594` `xor edi,edi`, `0x005b0b0e` `push 0;push 0`, `0x004386b6`). **Mérve: 60 elérhető valódi fájlból 16 bitpontos egyezés, RÉSZLEGES egyezés 0** — ez utóbbi a döntő kontroll. Negatívum: az `"originhash"` sztring a binárisban **egyszer** fordul elő, csak a kiírás hivatkozik rá ⇒ a Picasa név szerint nem olvassa vissza. ⛔ ~~**NYITVA:** *melyik* fájl bájtjait rögzíti a mentéskor (a mai képét vagy az eredetiét).~~ **LEZÁRVA (2026-09-09, #2790):** az **ELSŐ MEGISMERÉSKORI** bájtokat rögzíti; a mentés nem frissíti (`picasa-tartalomkulcs.md`, utolsó szakasz). ⛔ A mi `edit/save.py`-ünk **SHA-256-ot (64 karakter)** ír a `redo=` láncra — se a hossz, se a bemenet nem stimmel; a csere a nyitott kérdés miatt **külön jegy**, nem ezé a köré. A részletek: [`picasa-tartalomkulcs.md`](picasa-tartalomkulcs.md).
...
⛔ **2026-09-06 (a 20. kör ÖNHELYESBÍTÉSE) — a `.cxf` `scale` „megerősített negatívja” MEGDŐLT (#1412, `kollazs-eletciklus.md` 17.15).** A 20. kör azt írta, hogy a `Picasa3.exe`-ben **nincs** a kollázs-csomópont `scale`-jét számoló kód, `megerősített` fokkal — és megadta a saját cáfolatának feltételét is („ha egy új, sosem mentett kollázs `.cxf`-jében minden csomópont `scale=1.000000`”). ⭐ **A tulajdonos ugyanaznap megmérte**, három korábban soha nem mentett kollázzsal: **AI27 → 500 · AI28 → 256 · AI29 → 158** (az `AI6` ismert értéke 313). ⇒ **VAN írási út, amely SZÁMOL** — a két korábbi lehetőség közül a (2) áll: egy olyan út, amit **egyik pásztázásunk mintája sem fedett** (blokk-másolás, vagy a mentés-szervező `0x00834700` hívóláncának egy el nem olvasott ága). ⛳ **Hol volt a hiba:** a mérés és a pásztázás **helyes** (a 63 írási hely tényleg nem számol) — a **következtetés** ugrott a minták uniójáról „a programban nincs ilyen kód”-ra, holott a két minta csak a **közvetlen `fst`/`fstp`** írásokat fedi; a `memcpy`/`rep movsd` és a többi közvetett út **kimondatlanul kimaradt**. **A negatív pásztázás hatókörét a MEZŐRE kell szabni, nem a mintára.** ⚠️ **A négy mérési pont nem monoton** (6 → 256, de 9 → **313**), tehát egyszerű `f(képszám)` alak nincs; képletet illeszteni addig tilos, amíg a rács-, lap- és képarány-bemenetek nincsenek kiolvasva a `.cxf`-ekből. ⛔ **Új mintát kérni ugyanerre TILOS** (a tulajdonos kifejezett utasítása). Jegy: **#1412** (`ready`).

⭐ **2026-09-06 (21. kör) — a 85 HORGONYTALAN spec-szakasz felszámolva, és két hiba a MÉRŐBEN (#2536, 22.7).** A #2182 óta ismert adósságot (85 szakasz / 128 elem) szakaszonként elolvasva kiderült: a szakaszok **nem forrás nélküliek**, csak a horgony **ALAKJA** hiányzott — 27 idéz `.tre`-blokkot sorszám nélkül, 41-ben nincs semmilyen hivatkozás, 6 prózában adja meg („az `editpanel.tre` 585–588. sora"), 1 rövid alakú címmel (`0x5ccb36`), 1-ben pedig a hexa **ARGB szín** (`0xFF7D8397`), amit a `CIM_MINTA` szándékosan nem fogad el. ⭐ **76 szakasz kapott `*Forrás: …*` sort**, elemenként a `.tre`-deklarációval; mind a **157** (szakasz, elem) horgony **gépileg ellenőrizve**, hogy a hivatkozott sor tényleg az adott elem deklarációja. ⛔ **MÉRŐ-HIBA 1:** a ```-kerítésen belüli `#` sor (a `.tre` megjegyzés-jele) markdown-címsornak számított, és **kettévágta** a horgonyzott szakaszt — a 84-ből **öt** álszakasz volt; javítva, őr: `tests/test_lefedettseg_kerites_2536.py` (a publikus `tests/support/spec_horgony.py` szakaszolása is átvezetve, mert a kettőnek együtt kell mozognia). ⛔ **MÉRŐ-HIBA 2:** a **tartalomjegyzék bizonyítékként számított** — ez a lap körönként összefoglalja a MÁSIK lapok leletét, és a ⭐ bekezdések egyetlen `###` alá gyűlnek: 2026-09-06-án **55 257 karakter**, ami bármely benne szereplő elemnevet „igazolt" egy oda nem tartozó címmel, ráadásul ábécésorrendben elsőként **elnyelte a valódi forrást**. **24 elem** bizonyítéka volt így a `00-index.md`; most **0**. ⭐ **Mérve, előtte→utána:** horgonytalan szakasz **84 → 4** (mind a négy indokolt „Amit KIZÁRTAM"), `lekutatva` **294 → 297**, `bizonytalan` **47 → 44**, `feltáratlan` **0 → 0**. ⚠️ **A `feltáratlan` nem mozdult** — a kör a **bizonyítás minőségét** javította, nem a kutatói munkalistát rövidítette. Jegy: **#2536**.
...
| [picasa-arcfelismeres.md](picasa-arcfelismeres.md) | **Az arcfelismerés TELJES működése** — a három réteg és kapcsolóik, a két küszöb-létra, a KÉT ini-írási útvonal (`facedata`!), a `db3` arc-oszlopai élő adaton mérve, a három romboló művelet, a verzió-migráció; **15.: a személy-album fejlécsávja** — 25 parancs, a küszöb-lazítás képlete, a `]ignoreface` írás (#2187) · ⭐ **2026-09-24 (#3563, 9/b): az Emberek-panel feliratválasztója** — `0x00647df0`: egyképes / többképes ág, a „Szintén ezeken a fotókon:” a SZEMÉLY-ALBUM fejléce (nem második szakasz), „Ki látható ezeken a fotókon?”, betöltés- és ellenőrizetlen-feliratok, öt `instructions`-mód → fejlesztés #3566, al-kutatás #3565 |
...
| [ui-lefedettseg.md](ui-lefedettseg.md) | Az eredeti panelek ↔ a mi QML-fánk megfeleltetése · ⭐ **2026-09-06 (#2529): a megszűnt online elemek `nem-cel`-re.** A `ui-lefedettseg-elemek.csv` 191 tétele: **120 `megvan` · 55 `lekutatva` · 17 `nem-cél`**. A tizennégy új `nem-cél` mind a Picasa Web Albums feltöltéshez tartozik (`uploadmgr` 6, `headerpanel` 4, `acquirepanel` 4) — a viselkedésük mérve a `picasa-feltolteskezelo.md`-ben, a döntés helye a `help/features/meg-nem-erheto-el.md` és a #638. ⭐ **2026-09-06: a FELTÁRATLAN oszlop NULLA** — 31 → 16 → 11 → **0** három kör alatt (#2529 lezárva); nem cél 77 → 91, lekutatva 267 → **294**, bizonytalan 51 → **47**, lefedettség **41,2% → 42,1%**. ⛳ **Amit ez NEM jelent:** a `lekutatva` annyit tesz, hogy egy spec horgonyzott szakaszban, teljes néven megnevezi az elemet — nem azt, hogy a viselkedése végig ki van mérve; a 47 `bizonytalan` elem is nyitott munka. ⛔ **128 további elem azért látszana feltáratlannak, mert 85 spec-szakasz HORGONY NÉLKÜL nevezi meg őket** (#2536) — ez a hibafajta a #2529 három köréből NÉGYSZER jött elő. *(Előzmény, #2504: hat elavult kézi `hianyzik` felülbírálás javítva — `thumbui/hviewtoggle` `thumbui.tre:406`, `editpanel/movietab` `editpanel.tre:476`, `editpanel/movietabpanel` `editpanel.tre:484`, `editpanel/modaldialogblur` `editpanel.tre:1362`, `editpanel/picnikapply` `editpanel.tre:1430`, `editpanel/preview2` `editpanel.tre:1066`; ettől lett 37 → 31.)* |
| [picasa-feltolteskezelo.md](picasa-feltolteskezelo.md) | A Feltöltéskezelő (`CUploadManagerDialog`) hat vezérlője és a megszűnt Web Albums-elemek · a `throttlechk` az EGYETLEN tartós állapot (`Preferences\AutoBandwidthThrottle`, alapérték 0) |
...
|| [kollazs-panel-ui-spec.md](kollazs-panel-ui-spec.md) | **A Kollázs-panel MEGVALÓSÍTÁSI UI-specifikációja** — elemfa, `objectName`-ek, a `.tre` kényszereiből levezetett méretezési törvény, R2/R3 szerkezeti kontroll (a Beállítások-lap wrapper- és clip-réteg eltéréseivel, a négy vászon körüli csoport, a négy alsó globális gomb és a lap/csomópont-rétegek mért R3-egyezésével/R2-eltérésével, valamint a folyamatjelző belső geometriai eltérésével), **R5 felirat- és súgókontroll: a vászon körüli 21/21 magyar és 17/21 angol mező, a Beállítások-lap 47/48 karakterpontos magyar és 42/48 eredeti angol forrás-egyezés, a Klipek lap 3/3 magyar TS-forrása és 1/3 eredeti angol forrás-egyezése; a két Klipek-súgó eltérésének fejlesztői jegye #3410, a Beállításoké #3408**, vezérlő-API, teszt-szerződés, jegyekre bontás |
...
| [picasa-metaadat-tulajdonsagok.md](picasa-metaadat-tulajdonsagok.md) | ⭐ **2026-09-23 — az XMP `aux:Lens` elsőbbsége (#3496, 9.14):** a `ytXMPReader` a 255-ös kulcsot „első nyer” alapon tölti, és a betöltők az objektív-elosztó ELŐTT olvasnak XMP-t ⇒ ha van `aux:Lens`, az látszik; az `XMP::Lens` csak a panelfelirat szövegtár-azonosítója. ⭐ **2026-09-23 — a Tömörítés sor kód → szöveg táblája (#3535, 14.):** a `0x009f23a0` 36 kódot ismer (`EXIF::…` szövegtár-azonosítókkal), ismeretlenre a számot írja; ⚠️ az eredeti táblája a 0/1-nél elcsúszott (tömörítetlen TIFF → „CCITT 1D”). Nálunk 4 kód. ⭐ **2026-09-23 — a Nikon-objektívág teljes menete (#3495, 9.13):** kulcs = `LensData[6/0x0b/0x0c …+6]` + `LensType`, verziótól függő eltolással és visszafejtéssel; tartalék `A-Bmm f/X-Y`. ⛔ **A Nikon-tábla rekordja `{kulcs, név}`, 417 sor** — a 9.2 olvasata egy sorral elcsúszott, a termék táblájából 17/416 helyes. ✅ **2026-09-22 — a megnevezett arc NEM kerül `PersonInImage` alá** (12/E, #3424): a literál három hivatkozása olvasó/regisztráció, az arcrégió-író (`0x00bb17e0`) csak `mwg-rs` + `MP`. ⭐ **ÚJ (2026-09-05, #2375): a Picasa metaadat-kulcstere TELJES egészében.** A Picasa a kép EXIF/IPTC adatait **egyetlen, egész számmal kulcsolt szótárba** olvassa (`BinaryMetadata`, RTTI `0x00d402b4`; `GetString` = `0x009f05c0`, 143 hívási hely a `.text`-ben), és a program mindenhonnan ebből kérdez. ⭐ **A kulcs = a tulajdonságtábla `id` mezője + 1** — négy független hívási hely dönti el, a legerősebb a geocímke-hármas (`0x009f15e5`: `0x8c`/`0x8e` a SZÖVEGES lekérdezőn = GPSLatitudeRef/GPSLongitudeRef ASCII[2], `0x8d` a SZÁMSOROS `0x009f1250`-en = GPSLatitude RATIONAL[3]); a `+1` nélküli olvasat mindegyiken rossz lekérdezőt adna. **Teljes tábla kiírva:** 176 EXIF/TIFF-bejegyzés (`0x00c782f0`, 28 bájtos rekordok) + 55 IPTC-bejegyzés (`0x00c77c24`, 20 bájtos rekordok, a szabványos hosszkorlátokkal — 2:120 max 2000, 2:122 max 32), névtér-enummal (0 = IFD0, 1 = Exif, **2 = üres**, 3/4/5 = gyártói jegyzet *feltételesen* Canon/Nikon/Olympus, 24 = GPS, 25 = Interop, 26 = IFD1). ⭐ **A #2304-re:** a `0x37` = EXIF **`0x9003` DateTimeOriginal** (nem a szomszédos `0x9004` DateTimeDigitized!), a `0x68` = `0xa420` ImageUniqueID, a `0xe4` = **IPTC 2:120 Caption/Abstract**. ⛔ **Négy IPTC-sor ÜTKÖZŐ `id`-t visel** (2:60 és 2:62 → 55, 2:63 és 2:65 → 56, 2:85 → 29, 2:118 → 40) ⇒ ugyanaz a belső tulajdonság több forrásból is feltölthető. ✅ **MIT AD MA:** mind a négy vizsgált mezőt a `metadata/reader.py` **ugyanazon a címszámon** olvassa (`:41` 36867, `:59` 42016, `:63` (2,120), `:42–43` Make/Model) ⇒ **termékteendő nincs**. Jegy: **#2375**; ⛳ **2026-09-18 (#3345):** a kutatási határvonal `0x00bab6e0` címen **4153 bájtos** függvényt, **70** EXIF/GPS-nevet és nyolc segédfüggvény-hívást mért; a névregiszter pontos dispatch-szerepe **NINCS MEG**, a kutatási jegy nyitva marad. ⛳ **2026-09-19 (#3366):** a 14 rekeszes tábla olvasó-diszpécsere az `ytXMPReader` rétege (`0x00ba8f80`), a Tulajdonságok-panel viszont **egyetlen rekeszt sem olvas közvetlenül**: a `CPropertiesDlg` az `imagedata` belső rekordot használja. A hét korábban névtelen rekesz kulcsai és szerepei kimérve; a kutatási kérdés lezárva. ⛳ **2026-09-18 (#3348):** a határvonal `0x00bdbe50` címen külön XMP Core-blokkot mért: **1822 bájt**, közvetlen hívó `0x00bd9ae0`, **80** sztring és **48** namespace-URL; RTTI-ben külön `ytXMPReader`/`ytXMPWriter`/`XMP_NamespaceTable`/`XMPMeta` család áll. A tényleges namespace-regisztráló hívás és az URI→prefix leképezés a helyi EXE hiányában **NINCS MEG**; a #3348 nyitva marad · ⭐ **2026-09-24 (#3557, 15.): a Fényforrás (`LightSource`, 66-os kulcs) formázója** — `0x009f35d0`, 21 szabványos kód, a 16-os hiányzik, ismeretlenre a szám; ⛔ a panel **nem** mutatja: a kiírás csak a `properties.xml` láncán halad (`0x006372f8`), abban `<LightSource/>` nincs. A „Fehéregyensúly” sor a 94-es kulcs (Automatikus/Kézi, ismeretlenre a szám) |
...
| lap | miről szól |
|---|---|
| [binaris-regeszet-modszertan.md](binaris-regeszet-modszertan.md) | **A szerszámosláda**: mit hoz ki egy eszköz, és mit NEM lát; **22.5: a horgony-előírás megsértése 12 leírt elemet tett láthatatlanná** a lefedettségi mérésben (#2182); **22.7: a 85 horgonytalan szakasz felszámolva, és két MÉRŐ-hiba is kiderült** (#2536) |
...
⛔ **MÉRVE (2026-09-09): mégis elavult — 4 jelölés a 8-ból, azaz a FELE.** A
lista nyitott jelöléseit végigmértem a hivatkozott lapok saját szakaszlistája
ellen. *(A 241. kör hármat talált és javított; a negyediket akkor
ellenőrizetlenül hagytam, és a 242. kör mérése szerint az is elavult —
a saját auditom volt hiányos.)*

| jelölés | valóság |
|---|---|
| a `contactsheet` csempeméretének képlete | a `kollazs-eletciklus.md` **18.2**-ben megvolt (2026-09-06) |
| a `scale` írójának keresése „a sávon kívül, vagy mutatós írás" | a mutatós ÉS az indexelt út is negatív (#2805) — a gépi keresés kimerült |
| „melyik fájl bájtjait rögzíti a mentéskor" | LEZÁRVA (#2790, 2026-09-09) |
| a `facerect` ÍRÓJA „öt kizárt úttal" | a lap `✅ A facerect ÍRÓJA…` szakasza megválaszolta; **#2515 LEZÁRVA** |

Mindhárom **valódi kárt okozott**: a 239–240. kör azért nyitott újra egy már
megválaszolt kérdést, mert innen választott témát. A három sor most
áthúzva + elavulás-jelzővel áll (nem törölve — a történet marad).

⚠️ **Amit ez a szabályról mond:** a „zárd le, és vedd ki innen" sor **nincs
kapuval**, és emiatt rendszeresen kimarad. Amíg nincs mérője, a témát
választó kör **kötelezően ellenőrizze** a jelölést a lap SAJÁT
szakaszlistáján (`grep -n '^## ' <lap>`), mielőtt kört indít rá — a jelölés
maga nem bizonyíték.
