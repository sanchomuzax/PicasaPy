...
⭐ **2026-09-19 — #3390: a kétmegállós pont-LUT bájt- és képponti útja lezárva.** A `0x008f3700` csatornánként `TRUNC(c0 + t·(c1−c0))`-t ír, a két alfa-végponttal a 256 rekesz pontosan `[255,254,…,1,0]`; a `0x008f3970` a sugár fixpontos LUT-rekesz + 8 bites törtérték párral kever, `sqrtps`-sel négy képpontot dolgoz fel párhuzamosan, **nem** végez felülmintavételezést. A mai kontroll: 82 célzott próba zöld, `dot_size_for(1600)=24`, `DOT_SCALE=0,8`, `_EDGE_SOFTNESS_PX=1,0`. A teljes `DotContrast`-meredekség izolált natív goldenje **NINCS MEG**; következő irány: `MasterCurve`/`Pixelate`/`BW`/`GetVar … Mask` lánc célzott összevetése. Részletek: `filters-decoded.md`, #3390.
