...
| [ui-lefedettseg.md](ui-lefedettseg.md) | Az eredeti panelek ↔ a mi QML-fánk megfeleltetése · ⭐ **2026-09-06 (#2504): a mérés újrafuttatva, és a KÉZI FELÜLBÍRÁLÁSOK javítva.** A `ui-lefedettseg-elemek.csv` 178 tétele: **120 `megvan` · 55 `lekutatva` · 3 `nem-cél` · 0 `hiányzik`**. ⛔ **Az előző (2026-09-05-i) sor hibás volt: „1 `hiányzik`" helyett a fájlban HAT ilyen tétel állt** (`thumbui/hviewtoggle` — `thumbui.tre:406`; `editpanel/movietab` — `editpanel.tre:476`; `editpanel/movietabpanel` — `editpanel.tre:484`; `editpanel/modaldialogblur` — `editpanel.tre:1362`; `editpanel/picnikapply` — `editpanel.tre:1430`; `editpanel/preview2` — `editpanel.tre:1066`) — mind a hatnak volt bizonyítéka a saját `bizonyitek` oszlopában ÉS teljes néven, horgonyzott szakaszban leírt spec-szakasza, tehát mind a hat **`lekutatva`**, nem feltáratlan. Átsorolva. ⇒ **az öt vezető panel feltáratlan oszlopa most 0 / 0 / 0 / 0 / 0**, az összes feltáratlan **37 → 31**, a lefedettség **41,2%**. A megmaradt 31 feltáratlan elem MÁS paneleken ül (`acquirepanel`, `edittextpanel`, `headerpanel`, `uploadmgr`, `geopanel`, `movieeditpanel` …) — ezekből a kutatói kör TUD témát venni. |
