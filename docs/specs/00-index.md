...
### [filterdesc-registry.md](filterdesc-registry.md) — 2 NYITOTT + 1 BLOKKOLT tétel (NYITOTT/1: a #2746 — **miért nem az oktree-út eredménye kerül a képre a `QuantizePalette`-nál, holott a bináris oktree-t épít és a látható kimenet csatornánként egyenletes rácson ül?** A 221. kör két irányt KIZÁRT: a rács nem a `0x00bcb2f0` képponti alkalmazóban keletkezik (nincs benne osztás), és a binárisban nincs név szerinti megkerülő út (egyetlen sztring-hivatkozás, a regisztráló). A 222. kör a `4`-es kódot is KIZÁRTA: 2806 slot-6 hívóhely közül egyetlen sem vizsgálja (három találat, mind hamis pozitív, elolvasva) ⇒ a korai ág élesben nem fut, a FŐ ÚT fut. A maradék irány: mit ad a fő út a 3-3-2 LUT-tal — mérés, nem vezérlés-keresés. NYITOTT/2: a #2125 — az `Invert` csempéjén VAN kék jelvény, pedig `mode="effect"`; a jelvény feltétele mérve `mode==1`, a maradék kérdés a `[0x00d67f68]` szolgáltatás `vtbl+0x14`-e. A `filterdesc.xml`-kérés 2026-09-08-án FELOLDVA: a telepítés-mentésben megvan, bájtra azonos a miénkkel. BLOKKOLT: a #2456, a `PicnikFocalPixelate` HETES alakjának exportja)
