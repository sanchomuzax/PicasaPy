...
| `controller` | `setCollageSavedPath` | Slot | `app/collage_save.py` | #1109 — #1487 (döntés): ez a „.cxf-ből nyitás” horga (spec 3.2/9.2): a Projektek ▸ Kollázsok belépési pont adná át, MELYIK fájlból dolgozunk. Az a belépési pont még nincs megépítve, tehát a slot nem holt kód, hanem egy megnevezett jegyre vár — a kollázs-átadólevélben (#1109) ez a sor. |
...
| `controller` | `collageSeed` | Property | `app/create_controller.py` | BELSŐ — #1487 (döntés): a véletlen elrendezés magja. A `_collage_seed`-et a kollázs-készítő Pythonból használja (create_controller.py:244, :268, :316); a panel a KÉPET kapja, nem a magot. Felületi lekérdezésre nincs szükség. |
...
| `controller` | `setExportMovieFull` | Slot | `app/export_controller.py` | FELVÁLTVA — #1487 (döntés): a párbeszéd a TELJES beállítás-térképet adja át mentéskor (`ExportDialogs.qml:239`, `"movieFull"` kulcs), nem egyetlen setterrel ír. Az olvasója (`exportMovieFull`) már BELSŐ. |
| `controller` | `locationOfRow` | Slot | `app/geo_controller.py` | FELVÁLTVA — #1487 (döntés): a térkép a BŐVEBB `geoMarkers`-t kéri (`PlacesMap.qml:11` — `[{row, name, latitude, longitude}]`), ami soronként ugyanezt az adatot tartalmazza. Egy soros lekérdezésre nincs felületi igény; a próbái a koordináta-átadást ezen a szűkebb alakon mérik. |
...
| `editController` | `hasSavedRedeye` | Property | `app/edit_controller.py` | #2393 — MENTETT javítást jelez; az EditorPanel.qml:132 SAJÁT, MÁS jelentésű `redeyeActive`-ot tart (nyitott eszköz). #1487 (döntés): a bekötésről a #2393 dönt, ott derül ki, kell-e a felületnek a MENTETT állapot — addig nem törölhető, mert a próbái a szerkesztő állapotát ezen át olvassák. |
| `editController` | `hasRetouch` | Property | `app/edit_controller.py` | ELDÖNTVE (#1487) — a felület SZÁNDÉKOSAN mást mutat: a feliratot az `undoLabel` adja (#465), a csempe kiemelése a NYITOTT eszközt jelzi (#116) — mindkettő mérve az eredetihez képest (#1052). A property marad: a retusálás-próbák a szerkesztő állapotát ezen át olvassák, és a törlése a teszteket belső mezőkbe kényszerítené. |
...
| `startupStatus` | `busy` | Property | `app/startup_status.py` | ELDÖNTVE (#1487) — a nyitóképernyő SZÁNDÉKOSAN a saját `busy: !root.ready` alakját számolja (`SplashScreen.qml:42`): egyetlen forrásból (`ready`) vezeti le, így nem lehet a kettő ellentmondásban. A property marad, mert a `report`/`finish` (mindkettő BELSŐ) ugyanennek az objektumnak az indítási API-ja, és a próbák innen olvassák az állapotot. |
