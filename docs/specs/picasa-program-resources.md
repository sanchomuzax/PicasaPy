...
| `MovieThumb.exe` | Windows PE32 GUI EXE | Videófájlokból (a Picasa "Mozifilm" funkció exportjaiból) bélyegkép/előnézeti kép generáló segédprogram; parancssori kapcsolói között `/savemovie` és `/playmovie` látszik, valamint JFIF-thumbnail-beágyazási logika. |
...
| `qtsupport.dll` | Windows PE32 DLL | **Apple QuickTime-integráció** — stringek (`QuickTime.qts`, `theQuickTimeDispatcher`, `Software\Apple Computer, Inc.\QuickTime`) alapján a régi `.mov`/QuickTime-alapú videóformátumok dekódolásához/lejátszásához szükséges régi Windows-rendszerkomponens (Apple QuickTime for Windows) elérését hidalja át. |
| `npPicasa3.dll` (a gyökérben) | Windows PE32 DLL, Netscape-plugin (`np*`) elnevezési konvenció | Böngésző-beépülő modul (NPAPI) — feltehetően a Picasa Webalbumok böngészős feltöltő/integrációs funkcióihoz (kép-előnézet vagy fájlkiválasztó a böngészőben); az NPAPI 2015 körül már kivezetés alatt állt a legtöbb böngészőben. |
