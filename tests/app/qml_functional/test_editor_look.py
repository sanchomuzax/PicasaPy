...
class TestIconTabs:
    ...
    def test_active_tab_still_gets_a_highlighted_border(self, qml_engine, qt_app):
        ...
        assert active.property("border").color() != inactive.property("border").color()
...
class TestPanelButtonThumbnailPlaceholder:
    ...
    def test_thumb_source_shows_a_never_blank_placeholder_until_ready(
        ...
        # nincs provider regisztrálva ebben az izolált engine-ben -> Error,
        # sose Ready — ez pont az az eset, amit a helyőrző lefed
        assert thumb_img.property("status") != 1  # 1 == Image.Ready
        label = root.findChild(QObject, "btnLabel")
...
class TestEffectThumbnailIntegration:
    ...
    def test_thumbnail_arrives_asynchronously_without_blocking(
        ...
        while time.monotonic() < deadline and thumb_img.property("status") != 1:
            qt_app.processEvents()
        assert thumb_img.property("status") == 1, "a bélyegkép nem érkezett meg"
        assert 0 < thumb_img.property("implicitWidth") <= 100
        assert 0 < thumb_img.property("implicitHeight") <= 100

    def test_two_different_effect_thumbnails_are_visually_different(
        self, qml_app, qt_app
    ):
        """Ugyanaz a fotó, két eltérő effekt — a képadat NEM lehet azonos."""
        window, _controller, _engine = qml_app
        panel = self._open_effects_tab(window, qt_app)
        ...
        while time.monotonic() < deadline and (
            sepia_img.property("status") != 1 or bw_img.property("status") != 1
        ):
            qt_app.processEvents()
        assert sepia_img.property("status") == 1
        assert bw_img.property("status") == 1
