"""A „ne írj a felhasználó VALÓDI mappájába" őr felismerő logikája (#1054).

A `tests/conftest.py` autouse-fixture-e ezt használja. Azért él külön
modulban, hogy tesztelhető legyen: a `conftest` nevet több alkönyvtár is
viseli, egy `from conftest import …` a gyűjtésnél a rosszat találná meg.
...
mappában. A fájl ott állt, és a #1051 jegy „élő bizonyítékként" hivatkozott
rá, mint a TULAJDONOS elveszett munkájára — miközben a csomópontjai egy
`pytest` ideiglenes könyvtárára mutattak.

Egy szennyezés, ami egy jegy leletét hamisította meg.
"""
...
VEDETT_MAPPAK: tuple[Path, ...] = (Path.home() / "Pictures" / "Picasa",)


def pillanatkep(mappa: Path) -> dict[str, tuple[int, float]]:
...
def valtozas_szovege(
    mappa: Path,
    regi: dict[str, tuple[int, float]],
    uj: dict[str, tuple[int, float]],
) -> str:
